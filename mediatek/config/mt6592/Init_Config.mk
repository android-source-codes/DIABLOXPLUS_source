...
PRODUCT_COPY_FILES += $(MTK_ROOT_CONFIG_OUT)/audio_effects.conf:system/etc/audio_effects.conf
...
##### INSTALL jrdcom ##########
#PRODUCT_COPY_FILES += \
#                  $(TOP)/vendor/jrdcom/libs/libfota_d_native.so:system/lib/libfota_d_native.so \
#                  $(TOP)/vendor/jrdcom/libs/libstlport_shared.so:system/lib/libstlport_shared.so

_soul5_92_compass_cfg := $(MTK_ROOT_CONFIG_OUT)/configs/accel_nvm
ifeq ("soul5_92","$(TARGET_PRODUCT)")
ifneq ($(wildcard $(_soul5_92_compass_cfg)),)
PRODUCT_COPY_FILES += $(_soul5_92_compass_cfg):system/etc/accel_nvm
endif
endif

#Added by jrd.lipeng for smart pa parameters copying
########INSTALL Speaker and SmartPA parameters ########
ifeq ($(NXP_SMARTPA_SUPPORT),yes)
src_files := $(shell ls $(MTK_ROOT_CONFIG_OUT)/smartpa_params )
PRODUCT_COPY_FILES += $(foreach file, $(src_files), \
        $(MTK_ROOT_CONFIG_OUT)/smartpa_params/$(file):system/etc/smartpa_params/$(file))
endif
