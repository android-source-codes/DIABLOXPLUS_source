...
#define SENSORDB(fmt, arg...) if (debugFlag) {printk("[JRD_CAM][kd_sensorlist]"  fmt, ##arg);}


/******************************************************************************
 * Debug configuration
******************************************************************************/
#define PFX "[JRD_CAM][kd_sensorlist]"
#define PFX_ERR "[JRD_CAM_ERR][kd_sensorlist]"
...
/*******************************************************************************/
/*JRD camera sys debug begin */
/*******************************************************************************/
#define JRD_CAM_SYSFS_SUPPORT
#ifdef JRD_CAM_SYSFS_SUPPORT
bool  cam_otpdebug_flag = KAL_FALSE;
UINT32 kdGetSensorInitFuncList(ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT **ppSensorList);

static ssize_t sensor_debug_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char *s = buf;
    U8 val[4];
    s += sprintf(s, " usage::echo 1 > sensor_debug to open sensor log\n debugFlag=%d\n", debugFlag);
    return (s - buf);
}
static ssize_t sensor_debug_store(struct kobject *kobj, struct kobj_attribute *attr,
               const char *buf, size_t n)
{
	ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT *pSensorList_debug = NULL;
	SENSOR_FUNCTION_STRUCT *g_pInvokeSensorFunc_debug[2] = {NULL,NULL};
    int val;
	int num = 0;
    ssize_t ret;
    kal_uint8 i;
    ...
    if (0 != kdGetSensorInitFuncList(&pSensorList_debug)){
        PK_ERR("[JRD_CAM]ERROR:kdGetSensorInitFuncList()\n");
        return n;
    }
           
	for (num=0; num < MAX_NUM_OF_SUPPORT_SENSOR; num++){
		if (NULL == pSensorList_debug[num].SensorInit){
		    printk("[JRD_CAM][kd_sensorlist]sensorlist end\n");
		    break;
		}
		printk("[JRD_CAM][kd_sensorlist]sensorname=%s\n", pSensorList_debug[num].drvname);
		pSensorList_debug[num].SensorInit(&g_pInvokeSensorFunc_debug[0]);

	    if ( g_pInvokeSensorFunc_debug[0] ){
            ret = g_pInvokeSensorFunc_debug[i]->SensorFeatureControl(SENSOR_FEATURE_DEBUG_LOG, &val, 1);
        }else{
        	printk("[JRD_CAM][kd_sensorlist] sensorlist is NULL");
        }
	}
...
static ssize_t otp_debug_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    char *s = buf;
    U8 val[4];
    s += sprintf(s, " usage::echo 1 > otp_debug to open otp log\n debugFlag=%d\n", debugFlag);
	
    return (s - buf);
}
static ssize_t otp_debug_store(struct kobject *kobj, struct kobj_attribute *attr,
               const char *buf, size_t n)
{
    char *s = buf;
    int val;
    ...
    if(0 != val)
        cam_otpdebug_flag = TRUE;
    else
        cam_otpdebug_flag = FALSE;

    return n;
}
EXPORT_SYMBOL(cam_otpdebug_flag);

static DEVICE_ATTR(otp_debug, 0644,  otp_debug_show, otp_debug_store);
static DEVICE_ATTR(sensor_debug, 0644,  sensor_debug_show, sensor_debug_store);

static struct attribute *CAM_sysfs_attrs[] = {
    &dev_attr_sensor_debug.attr,
	&dev_attr_otp_debug.attr,
    NULL,
};
...
struct kobject *JRD_CAM_debug_kobj;
static int CAM_sysfs_init(void)
{ 
    JRD_CAM_debug_kobj = kobject_create_and_add("jrd_cam", NULL);
    if (JRD_CAM_debug_kobj == NULL){
		printk("falied to creat jrd_cam\n");
        return -ENOMEM;
    }

    return sysfs_create_group(JRD_CAM_debug_kobj, &CAM_attr_group);
}
static void CAM_sysfs_exit(void)
{
    sysfs_remove_group(JRD_CAM_debug_kobj, &CAM_attr_group);

    kobject_put(JRD_CAM_debug_kobj);
}

#endif
/*******************************************************************************/
/*JRD camera sys debug end */
/*******************************************************************************/
#endif
...
)
{
    ...
    if(_IOC_NONE == _IOC_DIR(a_u4Command)) {
        ...
        pBuff = kmalloc(2*(sizeof(MSDK_SCENARIO_ID_ENUM)+sizeof(ACDK_SENSOR_INFO_STRUCT)+sizeof(ACDK_SENSOR_CONFIG_STRUCT)),GFP_KERNEL);    
        if(NULL == pBuff) {
...
static int CAMERA_HW_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#if defined(SH_DIABLOXPLUS_SUPPORT)
	CAM_sysfs_init();//add by xiewei for camera debug
	#endif
...
{
	#if defined(SH_DIABLOXPLUS_SUPPORT)
	CAM_sysfs_exit();//add by xiewei for camera debug
	#endif
