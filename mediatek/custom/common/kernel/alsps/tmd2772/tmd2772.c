...
static int tmd2772_get_als_value(struct tmd2772_priv *obj, u16 als)
{
	...
	for(idx = 0; idx < obj->als_level_num; idx++)
	{
		if(als < obj->hw->als_level[idx])
		{
			...
			case 0: //<3
				if(idx<idx_old){
					if(als<(obj->hw->als_level[idx]-1)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
				}else if(idx>idx_old){
					if(als>(obj->hw->als_level[idx]+12)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
			...
			case 1://<40
				if(idx<idx_old){
					if(als<(obj->hw->als_level[idx]-15)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
			...
			case 5://<250
				if(idx<idx_old){
					if(als<(obj->hw->als_level[idx]-50)){
						break;
					}else{
						idx=idx_old;//als_level don't change;
					}
...
static ssize_t show_alsvalue_value(struct device_driver *ddri, char *buf)
{
	int lenth=0;
	struct tmd2772_priv *obj =  i2c_get_clientdata(tmd2772_i2c_client);
	lenth=snprintf(buf,PAGE_SIZE,"%d\n",obj->als);
	APS_LOG("The Lightsensor value is %d \n",obj->als);
	return lenth;
}
static DRIVER_ATTR(alsvalue,       S_IWUSR | S_IRUGO, show_alsvalue_value,NULL);

static struct driver_attribute *tmd2772_attr_list[] = {
	&driver_attr_cali,
	&driver_attr_alsvalue,
};
/*----------------------------------------------------------------------------*/
...
static int __init tmd2772_init(void)
{
	APS_FUN();
	printk("tmd2772_init");
	i2c_register_board_info(2, &i2c_tmd2772, 1); //i2c bus1;
/*************wwl add psensor get calibration data when calling    start*****************/
	#if defined(MTK_AUTO_DETECT_ALSPS)
