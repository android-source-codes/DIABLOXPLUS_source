...
/*----------------------------------------------------------------------------*/
int kxtj2_1009_gsensor_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sobj.self = obj;
	sobj.polling = 1;
	sobj.sensor_operate = kxtj2_1009_gsensor_operate;
...
#endif
{
	struct acc_hw *hw = kxtj_get_cust_acc_hw();
	GSE_FUN();
...
#if defined(MTK_AUTO_DETECT_ACCELEROMETER)
	if(kxtj2_1009_init_flag)
	{
		GSE_ERR("auto add driver error\n");
		return -1;
	}
#endif
