...
/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 begin*/
#include <linux/platform_device.h>
#include <linux/hwmsen_dev.h>
/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 end*/

static DEFINE_SPINLOCK(kdsensor_drv_lock);

#define SUPPORT_I2C_BUS_NUM1        0  //modeify by zhuqiang for PR576471  
...
********************************************************************************/
/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 begin*/
static int camera_exit=0;
static int f_camera_exit=0;
static int activ=0;
/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 end*/
...
{	
    printk("camera show function!\n");

    if (!dev ) {
       
        return 0;
    }
    printk("camera_show end \n");
    return snprintf(buf, PAGE_SIZE, "%d\n", f_camera_exit);    
   //Lightsensor flag,Let the APP know the Psensor is CTP type or physical type// add by llf
}

static ssize_t camera_store(struct device* dev, 
...
/*----------------------------------------------------------------------------*/
DEVICE_ATTR(frontcamera,     S_IWUSR | S_IRUGO, camera_show, camera_store);
/*----------------------------------------------------------------------------*/


static int camera_compatible_test(void)

{
	struct device * camera_device;

	camera_device=hwmsen_get_compatible_dev();
	if (device_create_file(camera_device, &dev_attr_frontcamera) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_frontcamera.attr.name);
	
	printk("camera activ is : %d \n",activ);
    ...
    if (g_pSensorFunc) {
		for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
			if(DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]){
				...
				/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 begin*/
				activ=activ|g_invokeSocketIdx[0];
		            PK_DBG(" activ =%x Sensor found ID = 0x%x\n", activ,sensorID);
				/*FR613228 add the compatible_device of sub camera by yinzhanyong 20140303 end*/
		            err = ERROR_NONE;
		        }
		        if(ERROR_NONE != err)
