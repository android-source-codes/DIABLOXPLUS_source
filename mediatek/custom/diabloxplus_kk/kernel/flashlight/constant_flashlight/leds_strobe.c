...
#define PK_DBG(fmt, arg...)  if(debugFlag) {printk(KERN_INFO "[JRD_CAM][MT6333]"  fmt, ##arg);}
#define PK_ERR(fmt, arg...)  			   {printk(KERN_ERR "[JRD_CAM][MT6333]"  fmt, ##arg);}
...
static int FL_preOn(void)
{
	...
	if ( debugFlag ){
        reg = getReg(0x35);
		PK_DBG("FL_preOn- if read 0x35 reg=%d\n", reg);
        reg = getReg(0x00);
		PK_DBG("FL_preOn- if read 0x00 chip id=%d\n", reg);		
        reg = getReg(0xFD);
		PK_DBG("FL_preOn- if read 0xFD chip id=%d\n", reg);			
	}
...
{
	printk("[JRD_CAM][flashlight]FL_Enable+ g_duty= %d\n", g_duty);
...
static int flashlight_debug(int val)
{
	printk("flashlight_debug val=%d\n", val);
...
static int constant_flashlight_ioctl(MUINT32 cmd, MUINT32 arg)
{
    ...
    switch(cmd)
    {
		...
		case FLASHLIGHT_ENABLE_LOG:
			flashlight_debug(arg);
			break;

        default :
    		PK_DBG("flashlight_defalut No such command \n");
    		i4RetValue = -EPERM;
    		break;
    }
    return i4RetValue;
...
static int constant_flashlight_open(void *pArg)
{
    ...
    if(strobe_Res)
    {
        PK_ERR("constant_flashlight_open busy!\n");
        i4RetValue = -EBUSY;
    }
    else
    {
    ...
    PK_DBG("constant_flashlight_open line=%d i4RetValue=%d\n", __LINE__ , i4RetValue);
...
//added by xiewei
int mt6333_read_id_reg(void)
{
	int reg_id1 = 0, reg_id2 = 0;

	reg_id1 = getReg(0x00); 	
	reg_id2 = getReg(0xFD);
	printk("[JRD_CAM][flashlight]reg_id1=%x reg_id2=%x\n", reg_id1, reg_id2);
	if ( (reg_id1 == 0x93) ){
		printk("[JRD_CAM][flashlight]driver is mt6333\n");
		return reg_id1;
	}else{
		printk("[JRD_CAM][flashlight]driver doesn't mt6333\n");
		return -1;
	}
}
