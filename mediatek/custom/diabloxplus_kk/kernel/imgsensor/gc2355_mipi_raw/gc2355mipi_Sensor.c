...
#define SENSORDB(fmt, arg...) 	   if (debugFlag) {printk("[JRD_CAM][GC2355BLX]"  fmt, ##arg);}
#define SENSORDB_ERR(fmt, arg...)  {printk("[JRD_CAM_ERR][GC2355BLX]"  fmt, ##arg);}
...
static int GC2355DebugLog(kal_bool value)
{
    printk(KERN_INFO "[JRD_CAM]GC2355DebugLog value is %d\n", value);
    if( 1 == value ){
        debugFlag = KAL_TRUE;
    }
}
...
kal_uint16  GC2355MIPI_SetGain(kal_uint16 iGain)
{
	...
	if((ANALOG_GAIN_1<= iReg)&&(iReg < ANALOG_GAIN_2))
	{
		...
		SENSORDB("GC2355 analogic gain 1x , GC2355 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
		...
		GC2355MIPI_write_cmos_sensor(0xb1, temp>>6);
		GC2355MIPI_write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		SENSORDB("GC2355 analogic gain 1.45x , GC2355 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
		...
		GC2355MIPI_write_cmos_sensor(0xb1, temp>>6);
		GC2355MIPI_write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		SENSORDB("GC2355 analogic gain 2.02x , GC2355 add pregain = %d\n",temp);	
	}
...
static void GC2355MIPI_Sensor_Init(void)
{
	...
	GC2355MIPI_write_cmos_sensor(0xf7,0x31); //19 //pll enable
	GC2355MIPI_write_cmos_sensor(0xf8,0x06); //Pll mode 2  /////86--暗处降频
	GC2355MIPI_write_cmos_sensor(0xf9,0x0e); //de//[0] pll enable
	GC2355MIPI_write_cmos_sensor(0xfa,0x00); //div
	GC2355MIPI_write_cmos_sensor(0xfc,0x06); //4e
	//GC2355MIPI_write_cmos_sensor(0xfd,0x00);
	...
	GC2355MIPI_write_cmos_sensor(0x0e,0xc0); //c0
	GC2355MIPI_write_cmos_sensor(0x0f,0x06); 
	GC2355MIPI_write_cmos_sensor(0x10,0x50); //Window setting 1616x1216
	/*GC2355MIPI_write_cmos_sensor(0x11,0x00);
	GC2355MIPI_write_cmos_sensor(0x12,0x18); //sh_delay
	GC2355MIPI_write_cmos_sensor(0x13,0x11);
	GC2355MIPI_write_cmos_sensor(0x14,0x01);
	GC2355MIPI_write_cmos_sensor(0x15,0x00);
	GC2355MIPI_write_cmos_sensor(0x16,0xc1);*/
	GC2355MIPI_write_cmos_sensor(0x17,0x17);//14
	//GC2355MIPI_write_cmos_sensor(0x18,0x02);
	GC2355MIPI_write_cmos_sensor(0x19,0x0b);
	GC2355MIPI_write_cmos_sensor(0x1b,0x49); //48
	GC2355MIPI_write_cmos_sensor(0x1c,0x12);
	GC2355MIPI_write_cmos_sensor(0x1d,0x10); //double reset
	...
	GC2355MIPI_write_cmos_sensor(0x22,0xa0); //e0   //80  //f0
	GC2355MIPI_write_cmos_sensor(0x23,0x51); //51
	GC2355MIPI_write_cmos_sensor(0x24,0x19); //0b //55
	GC2355MIPI_write_cmos_sensor(0x27,0x20);
	GC2355MIPI_write_cmos_sensor(0x28,0x00);//10
	GC2355MIPI_write_cmos_sensor(0x2b,0x81); //00 sf_s_mode FPN
	GC2355MIPI_write_cmos_sensor(0x2c,0x38); //50 //5c ispg FPN
	GC2355MIPI_write_cmos_sensor(0x2e,0x16); //05//eq width
	GC2355MIPI_write_cmos_sensor(0x2f,0x14); //[3:0]tx_width 写0能改麻点
	...
	/*	BLK	 */
	GC2355MIPI_write_cmos_sensor(0x18,0x02);//1a-lily
	GC2355MIPI_write_cmos_sensor(0x1a,0x01);//09-lily //01
	GC2355MIPI_write_cmos_sensor(0x40,0x42);//43-lily
	...
	GC2355MIPI_write_cmos_sensor(0x4e,0x3c); //BLK select
	GC2355MIPI_write_cmos_sensor(0x4f,0x00); 
	GC2355MIPI_write_cmos_sensor(0x5e,0x00);//18-lily //offset ratio
	GC2355MIPI_write_cmos_sensor(0x66,0x20);//20-lily //dark ratio
	GC2355MIPI_write_cmos_sensor(0x6a,0x02); //manual offset
	GC2355MIPI_write_cmos_sensor(0x6b,0x02);
	GC2355MIPI_write_cmos_sensor(0x6c,0x00);
	GC2355MIPI_write_cmos_sensor(0x6d,0x00);
	GC2355MIPI_write_cmos_sensor(0x6e,0x00);
	GC2355MIPI_write_cmos_sensor(0x6f,0x00);
	GC2355MIPI_write_cmos_sensor(0x70,0x02);
	GC2355MIPI_write_cmos_sensor(0x71,0x02);

	/* Dark sun */
	GC2355MIPI_write_cmos_sensor(0x87,0x03); //
	GC2355MIPI_write_cmos_sensor(0xe0,0xe7); //dark sun en/extend mode
	GC2355MIPI_write_cmos_sensor(0xe3,0xc0); //clamp

/*MIPI*/
  GC2355MIPI_write_cmos_sensor(0xfe, 0x03);
  GC2355MIPI_write_cmos_sensor(0x01, 0x87);
  GC2355MIPI_write_cmos_sensor(0x02, 0x11);
  GC2355MIPI_write_cmos_sensor(0x03, 0x91);
  GC2355MIPI_write_cmos_sensor(0x04, 0x01);
  GC2355MIPI_write_cmos_sensor(0x05, 0x00);
  GC2355MIPI_write_cmos_sensor(0x06, 0xa2);
  GC2355MIPI_write_cmos_sensor(0x10, 0x91);//91//94//1lane raw8
  GC2355MIPI_write_cmos_sensor(0x11, 0x2b);//2b
  GC2355MIPI_write_cmos_sensor(0x15, 0x60);
  GC2355MIPI_write_cmos_sensor(0x12, 0xd0); //d0//40
  GC2355MIPI_write_cmos_sensor(0x13, 0x07); //07//06
...
//  GC2355MIPI_write_cmos_sensor(0x27, 0x06);
  GC2355MIPI_write_cmos_sensor(0x29, 0x02);//04
...
UINT32 GC2355MIPIOpen(void)
{
	...
	printk("[JRD_CAM]GC2355MIPIOpen, sensor_id:%x \n",sensor_id);
...
UINT32 GC2355MIPIGetSensorID(UINT32 *sensorID) 
{
	...
	printk("[JRD_CAM]GC2355MIPIGetSensorID enter,\n");

	do {
		*sensorID=((GC2355MIPI_read_cmos_sensor(0xf0) << 8) | GC2355MIPI_read_cmos_sensor(0xf1)); 
		printk("[JRD_CAM]GC2355MIPIGetSensorID sensor_id:%x \n",*sensorID);

		if ( GC2355MIPI_SENSOR_ID == *sensorID )
			break;
		retry--;
	} while (retry > 0);
	...
	{
		 printk("[JRD_CAM]OV2680GetSensorID, sensor_id:%x \n",*sensorID);
		 *sensorID = 0xFFFFFFFF;
		 return ERROR_SENSOR_CONNECT_FAIL;
	}
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_DEBUG_LOG:
			GC2355DebugLog((kal_bool)*pFeatureData16); 
			break;
...
UINT32 GC2355MIPISensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc) 
{
