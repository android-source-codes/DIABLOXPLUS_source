...
#define SENSORDB(fmt, arg...) 	   if (debugFlag) {printk("[JRD_CAM][OV2680]"  fmt, ##arg);}
#define SENSORDB_ERR(fmt, arg...)  {printk("[JRD_CAM_ERR][OV2680]"  fmt, ##arg);}
...
kal_uint16 OV2680_read_cmos_sensor(kal_uint32 addr)
{
    ...
    iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, OV2680_SLAVE_READ_ID_1);
...
kal_uint16 OV2680_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
    ...
    iWriteRegI2C(puSendCmd , 3, OV2680_SLAVE_WRITE_ID_1);
    return ERROR_NONE;
}

int OV2680DebugLog(kal_bool value)
{
    printk(KERN_INFO "[JRD_CAM]OV2680DebugLog value is %d\n", value);
...
void OV2680SetFlipMirror(kal_int32 imgMirror)
{
	kal_int16 mirror=0,flip=0;
    flip = OV2680_read_cmos_sensor(0x3820);
	mirror   = OV2680_read_cmos_sensor(0x3821);
    switch (imgMirror)
    {
        case IMAGE_NORMAL:
            OV2680_write_cmos_sensor(0x3820, (flip & (0xFB)));//Set normal 0xBD--->0xbc  for capture size
            OV2680_write_cmos_sensor(0x3821, (mirror  & (0xFB)));	//Set normal  0xf9-->0xf8  for capture size
            break;
        case IMAGE_H_MIRROR:
            OV2680_write_cmos_sensor(0x3820, (flip & (0xFB)));//Set normal  0xbd--->0xbc  for capture size
            OV2680_write_cmos_sensor(0x3821, (mirror  | (0x04)));	//Set mirror
            break;
        case IMAGE_V_MIRROR:
            OV2680_write_cmos_sensor(0x3820, (flip |(0x04)));	//Set flip
            OV2680_write_cmos_sensor(0x3821, (mirror  & (0xFB)));	//Set normal //0xf9-->0xf8  for capture size
            break;
        case IMAGE_HV_MIRROR:
            OV2680_write_cmos_sensor(0x3820, (flip |(0x4)));	//Set flip
            OV2680_write_cmos_sensor(0x3821, (mirror  |(0x04)));	//Set mirror
            break;
    }
}
...
UINT32 OV2680Open(void)
{
	...
	do {
	   ...
	   printk("[JRD_CAM]OV2680Open, sensor_id:%x \n", sensor_id);
    ...
    {
        SENSORDB_ERR("OV2680 Check ID fails! \n");
		SENSORDB_ERR("[Warning]OV2680GetSensorID, sensor_id:%x \n",sensor_id);
...
UINT32 OV2680GetSensorID(UINT32 *sensorID) 
{
   int  retry = 2;
 	...
 	printk("[JRD_CAM]OV2680GetSensorID enter,\n");
        ...
        MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//filp on
	OV2680SetFlipMirror(IMAGE_HV_MIRROR);
    ...
    MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    printk("[JRD_CAM]OV2680ZsdPreview \n");
        ...
        MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    printk("[JRD_CAM]OV2680Capture start test1\n");
...
{
    printk("[JRD_CAM]OV2680GetResolution \n");
...
{
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680GetInfo\u62e2\u5362FeatureId:%d\n",ScenarioId);
#endif
...
{
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680Control\u62e2\u5362ScenarioId:%d\n",ScenarioId);
#endif	
...
#ifdef OV2680_DRIVER_TRACE
    SENSORDB("OV2680SetVideoMod 2FrameRate:%d\n",u2FrameRate);
#endif	
...
#ifdef OV2680_DRIVER_TRACE
    //SENSORDB("OV2680FeatureControl\u62e2\u5362FeatureId:%d\n",FeatureId); 
#endif		
    switch (FeatureId)
    {
		...
		case SENSOR_FEATURE_DEBUG_LOG:
			OV2680DebugLog((kal_bool)*pFeatureData16); 
			break;
