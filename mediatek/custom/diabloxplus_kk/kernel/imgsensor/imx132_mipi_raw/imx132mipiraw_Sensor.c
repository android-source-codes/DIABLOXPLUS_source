...
#define SENSORDB(fmt, arg...) 	   if (debugFlag) {printk("[JRD_CAM][IMX132]"  fmt, ##arg);}
#define SENSORDB_ERR(fmt, arg...)  {printk("[JRD_CAM_ERR][IMX132]"  fmt, ##arg);}
...
#else
#define IMX132MIPI_PREVIEW_CLK 110400000  //65000000
#define IMX132MIPI_CAPTURE_CLK 110400000  //117000000  //69333333
#define IMX132MIPI_ZSD_PRE_CLK 110400000 //117000000
#define IMX132MIPI_VIDEO_CLK   110400000 //117000000
#endif
...
int IMX132DebugLog(kal_bool value)
{
    printk("[JRD_CAM]value is %d", value);
...
//#ifdef IMX132MIPI_DEBUG_SETTING
	...
	//IMX132MIPI_write_cmos_sensor(	     ,     );
	//IMX132MIPI_write_cmos_sensor(//////, ////);////////////////////Address	value
	IMX132MIPI_write_cmos_sensor(0x0340, 0x06);
	IMX132MIPI_write_cmos_sensor(0x0341, 0x40);//frame length 1600
	...
	IMX132MIPI_write_cmos_sensor(0x3104, 0x30);
	IMX132MIPI_write_cmos_sensor(0x3105, 0x00);
	IMX132MIPI_write_cmos_sensor(0x3106, 0xCA);
	...
	IMX132MIPI_write_cmos_sensor(0x3301, 0x00);
	...
	mdelay(10); 
  	...
  	for(i = 0; i <(sizeof(sccb_writeid)/sizeof(sccb_writeid[0])); i++)
  	{
	   ...
	   sensor_id = ((IMX132MIPI_read_cmos_sensor(0x0000) << 8) | IMX132MIPI_read_cmos_sensor(0x0001));
		...
		printk("[JRD_CAM]IMX132MIPIOpen, sensor_id:%x \n",sensor_id);

		if(IMX132MIPI_SENSOR_ID == sensor_id)
		{
			printk("[JRD_CAM]IMX132MIPI slave write id:%x \n",IMX132MIPI_sensor.write_id);
			break;
		}
  	}
	...
	for(i = 0; i <(sizeof(sccb_writeid)/sizeof(sccb_writeid[0])); i++)
  	{
		...
		printk("[JRD_CAM]IMX132MIPIGetSensorID, sensor_id:%x \n",*sensorID);
...
////wenyuan.li@tcl 20130624 bug476960 merge begin]zhfan pr 446515
void IMX132MIPISetFlipMirror(kal_int32 imgMirror)
{
    kal_uint8  iTemp; 

    iTemp = IMX132MIPI_read_cmos_sensor(0x0101) & 0xFB;	//Clear the mirror and flip bits.
    switch (imgMirror)
    {
        case IMAGE_NORMAL:
            IMX132MIPI_write_cmos_sensor(0x0101, iTemp);	//Set normal
            break;
        case IMAGE_V_MIRROR:
            IMX132MIPI_write_cmos_sensor(0x0101, iTemp | 0x01);	//Set flip
            break;
        case IMAGE_H_MIRROR:
            IMX132MIPI_write_cmos_sensor(0x0101, iTemp | 0x02);	//Set mirror
            break;
        case IMAGE_HV_MIRROR:
            IMX132MIPI_write_cmos_sensor(0x0101, iTemp | 0x03);	//Set mirror and flip
            break;
    }
}
//[end]zhfan pr 446515
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    //[begin]//wenyuan.li@tcl 20130624 bug476960 merge zhfan pr 446515
    IMX132MIPISetFlipMirror(IMAGE_HV_MIRROR);
    ////wenyuan.li@tcl 20130624 bug476960 merge [end]zhfan pr 446515
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    //[begin]//wenyuan.li@tcl 20130624 bug476960 merge zhfan pr 446515
    IMX132MIPISetFlipMirror(IMAGE_HV_MIRROR);
    //[end]//wenyuan.li@tcl 20130624 bug476960 merge zhfan pr 446515
	mdelay(50);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	//wenyuan.li@tcl 20130624 bug476960 merge  s
	return ERROR_NONE;
	//wenyuan.li@tcl 20130624 bug476960 merge  e
}	/* IMX132MIPIControl() */
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_DEBUG_LOG:
			IMX132DebugLog((kal_bool)*pFeatureData16); 
			break;
