...
#include "../../../../mediatek/platform/mt6592/kernel/drivers/power/fan5405.h"
...
/*JRD BSP START*/
#if defined(EOS_3G_CHARGING_TEMPERATURE_PLOICY)
 ...
 int Stop_Charging_Above_4095 = 0;
 int Full_status_check = 0;
#endif
/*JRD BSP END*/
#if defined(DIABLOXPLUS_CHARGING_TEMPERATURE_PLOICY)
 int charge_temperature_above_step1 = 0;
 int charge_temperature_above_step2 = 0;
 int charge_temperature_above_step3 = 0;
 int charge_temperature_above_step1_valid = 0;
 ...
 int charge_temperature_above_step3_valid = 0;
 int Stop_Charging_Above_4095 = 0;
 extern int Full_status_check;
#endif
 int max_charge_tempertaure = 0;
int g_bat_voltage_above4095 = 0;
int min_charge_tempertaure = 0;

int weak_charger_time_sp = 0;
int weak_charger_time_not_sp = 0;
int weak_charger = 0;
int weak_charger_done = 0;
...
#define TURN_ON_CHARGING_FOR_FACTORY _IOW('k', 8, int)
//add for meta tool-----------------------------------------
...
int g_factory_charging = 0;
int byd_sucd_batt_id = 1; //1 BYD 2 SUCD
#define TOPOFF_CHG_VOLTAGE      4270
///////////////////////////////////////////////////////////////////////////////////////////
//// Thread related 
///////////////////////////////////////////////////////////////////////////////////////////
...
#ifdef DIABLOXPLUS_BATT_ID_SUPPORT
#define AUXADC_BATT_ID_CHANNEL 0
/*hongxing battery*/
#define CHG_MIN_HX_BATT_ID  780
#define CHG_MAX_HX_BATT_ID  980
/*jinneng battery*/
#define CHG_MIN_JN_BATT_ID  70
#define CHG_MAX_JN_BATT_ID  270
/*scud battery*/
#define CHG_MIN_SCUD_BATT_ID  1300
#define CHG_MAX_SCUD_BATT_ID  1510
/*BYD BATTERY */
#define CHG_MIN_BYD_BATT_ID   400
#define CHG_MAX_BYD_BATT_ID   600
int hx_jn_batt_id = 0; //1 HX 2 JN 3 SCUD 4 BYD
...
struct battery_data {
    ...
    int BAT_batt_vol;
    int BAT_batt_temp;
    int BAT_batt_id; //[FR571989][add battery id] by [wangxingxing] at [20140212] 
    /* Add for EM */
...
static enum power_supply_property battery_props[] = {
    ...
    POWER_SUPPLY_PROP_batt_id, //[FR571989][add battery id] by [wangxingxing] at [20140212] 
    /* Add for EM */
...
//// extern function
///////////////////////////////////////////////////////////////////////////////////////////
extern void mt_power_off(void);/*[FR565400][IEEE1725 requirement] add by [wangxingxing] at [20140213] */
...
void check_battery_id(void){
	...
	if((bat_id_vol < CHG_MAX_HX_BATT_ID)&&(bat_id_vol > CHG_MIN_HX_BATT_ID))
	{
		//BMT_status.bat_id_valid = 1;
		hx_jn_batt_id=1; //HX
		printk("battery id HX %d,between %d and %d !\n", bat_id_vol,CHG_MIN_HX_BATT_ID,CHG_MAX_HX_BATT_ID);
	}
	else if ((bat_id_vol < CHG_MAX_JN_BATT_ID)&&(bat_id_vol > CHG_MIN_JN_BATT_ID))
	{
		//BMT_status.bat_id_valid = 1;
		hx_jn_batt_id=2; //JN
		printk("battery id JN %d,between %d and %d !\n", bat_id_vol,CHG_MIN_JN_BATT_ID,CHG_MAX_JN_BATT_ID);
	}
	else if((bat_id_vol < CHG_MAX_SCUD_BATT_ID)&&(bat_id_vol > CHG_MIN_SCUD_BATT_ID))
	{
		//BMT_status.bat_id_valid = 1;
		hx_jn_batt_id=3; //SCUD
		printk("battery id SCUD %d,between %d and %d !\n", bat_id_vol,CHG_MIN_SCUD_BATT_ID,CHG_MAX_SCUD_BATT_ID);
	}
	else
	{
		hx_jn_batt_id=0; //illegal battery id
		printk("battery id illegal bat_id_vol== %d\n", bat_id_vol);
	}
}
#endif

//Add by Luoxingxing to adjust TP
#ifdef EOS_3G_CHARGING_TEMPERATURE_PLOICY
extern void tpd_enable_change_scanning_frq_(void);
extern void tpd_disable_change_scanning_frq_(void);
#endif
...
void reset_charging_iocharger(void)
{
	fan5405_reg_config_interface(0x04,0x0a);
}
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_present_2nd :
        ...
        break;
    case POWER_SUPPLY_PROP_batt_id:          //[FR571989][add battery id] by [wangxingxing] at [20140212] 
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
    ...
    bat_data->BAT_batt_id=BMT_status.id_vol;  //[FR571989][add battery id] by [wangxingxing] at [20140212] 
    /* Dual battery */
...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
   ...
   if( ((BMT_status.UI_SOC - BMT_status.SOC) == 1) && ((BMT_status.UI_SOC!=1)) )
   {
       BMT_status.UI_SOC = BMT_status.SOC;
   }    
   else if( (BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC!=1)) )
    {   
        //reduce after xxs
...
/*[FR571989][add battery id] by [wangxingxing] at [20140212] begin*/
...
kal_int32 battery_meter_get_battery_id_voltage(void)
{
   int i = 0, bat_id_vol = 0, data[4] = {0,0,0,0};
   int res =0;
   int rawdata=0;
...
#endif
/*[FR571989][add battery id] by [wangxingxing] at [20140212] end*/
void mt_battery_GetBatteryData(void)
{ 
	kal_uint32 bat_vol, charger_vol, Vsense, ZCV; 
	kal_uint32 bat_id_vol; //[FR571989][add battery id] by [wangxingxing] at [20140212]
	...
	temperatureR = battery_meter_get_tempR(temperatureV);
       #if BATT_ID_CHECK_SUPPORT
	bat_id_vol = battery_meter_get_battery_id_voltage(); //[FR571989][add battery id] by [wangxingxing] at [20140212]
       #endif
	...
	BMT_status.SOC = SOC;	
	BMT_status.ZCV = ZCV;
	BMT_status.id_vol = bat_id_vol ; //[FR571989][add battery id] by [wangxingxing] at [20140212]
...
/*[FR565400][IEEE1725 requirement] add by [wangxingxing] at [20140213] start*/
#if BATT_ID_CHECK_SUPPORT
#define CHG_MIN_VALID_BATT_ID_BYD  400
#define CHG_MAX_VALID_BATT_ID_BYD  600
#define CHG_MIN_VALID_BATT_ID_SCUD  1300
#define CHG_MAX_VALID_BATT_ID_SCUD  1510
static PMU_STATUS mt_battery_CheckBatteryId(void)
{	
	PMU_STATUS status = PMU_STATUS_OK;
  
    if((BMT_status.id_vol < CHG_MIN_VALID_BATT_ID_BYD) || ((BMT_status.id_vol > CHG_MAX_VALID_BATT_ID_BYD)&&(BMT_status.id_vol < CHG_MIN_VALID_BATT_ID_SCUD))||(BMT_status.id_vol > CHG_MAX_VALID_BATT_ID_SCUD)) /*battery id error*/
    {
...
#endif
/*[FR565400][IEEE1725 requirement] add by [wangxingxing] at [20140213] end*/
...
{	
	...
	/*JRD BSP START*/
	//#if defined(DIABLOXPLUS_CHARGING_TEMPERATURE_PLOICY)
    #if defined(DIABLOXPLUS_CHARGING_TEMPERATURE_PLOICY) || defined(EOS_3G_CHARGING_TEMPERATURE_PLOICY)
    /*JRD BSP END*/
    ...
    if(charge_temperature_above_step2 == 1)
    {
...
/*
    if(max_charge_tempertaure ==1)
    {
        if( BMT_status.temperature < HIGH_TEMPERATURE_RECOVER)
        {
            max_charge_tempertaure = 0;
        }
        else
        {
	...
	if(min_charge_tempertaure ==1)
    {
        if( BMT_status.temperature < LOW_TEMPERATURE_RECOVER)
        {
            min_charge_tempertaure = 0;
        }
        else
        {
            BMT_status.bat_charging_state = CHR_ERROR;
...
*/
	if(charge_temperature_above_step3 == 1)
	{
		...
		if(BMT_status.bat_vol <= 3985)
		{
			g_bat_voltage_above4095 = 0;
			max_charge_tempertaure = 0;
			Stop_Charging_Above_4095 = 0;
		}
		if(g_bat_voltage_above4095 == 1)
		{
			Stop_Charging_Above_4095 = 1;
		}
   ...
   if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)
    {
	max_charge_tempertaure = 1;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Over Temperature !!\n\r");                
        status = PMU_STATUS_FAIL;
    }
	if (BMT_status.temperature >= MIN_CHARGE_TEMPERATURE)
    {
	min_charge_tempertaure = 1;
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Temperature Too Low !!\n\r");
        status = PMU_STATUS_FAIL;
    }
    */
#endif
...
static void mt_battery_CheckUIShow(void)
{
	if( upmu_is_chr_det() == KAL_TRUE)
    {
           if((BMT_status.SOC == 99)&&(BMT_status.bat_vol >= TOPOFF_CHG_VOLTAGE))
           {
                BMT_status.bat_charging_state = CHR_TOP_OFF;
                BMT_status.TOPOFF_charging_time += BAT_TASK_PERIOD;
		printk("JRD  BMT_status.TOPOFF_charging_time=%d\n",BMT_status.TOPOFF_charging_time);

            }
           else if(BMT_status.SOC <= 98)
           {
               //BMT_status.bat_charging_state = CHR_CC;
               BMT_status.TOPOFF_charging_time = 0;
		printk("JRD  reset the BMT_status.TOPOFF_charging_time to 0\n");
           }
	}

}
static void weak_charger_detect(void)
{
	if((BMT_status.bat_charging_state == CHR_CC) && (weak_charger_done==0)&&( BMT_status.charger_type!=STANDARD_HOST))
	{
	kal_uint8 v5=0;
	int j = 0;
	//battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,CHARGE_CURRENT_750_00_MA);
	fan5405_reg_config_interface(0x04,0x20);
	msleep(300);
	for (j=0;j<2;j++)
	{
	printk("  ---------------------------------------------\n");
	//v5 = fan_struct->ops->get_reg_value(fan_struct, 0x05);
	v5=fan5405_get_sp_status();
	printk(" ---------------------------------------------\n");
	printk("  fan5405_reg5.. = %x\n",v5);

	if ((weak_charger_time_sp < 4) && (v5==1))
	{
		weak_charger_time_sp++;
	}
	else
	{
		weak_charger_time_sp = 0;
	}

	if ((weak_charger_time_not_sp < 4) && (v5==0))
	{
		weak_charger_time_not_sp++;
	}
	else
	{
		weak_charger_time_not_sp = 0;
	}
	}
	printk(" weak_charger_time=%d\n,weak_charger_time_not_sp=%d\n,weak_charger_done=%d\n",weak_charger_time_sp,weak_charger_time_not_sp,weak_charger_done);
	if(weak_charger_time_sp >= 4 || weak_charger_time_not_sp>=4)
	{
		if(v5 == 1)
		{
			//g_BatteryNotifyCode |= 0x0008;
			weak_charger = 1;
			//printk("g_BatteryNotifyCode = %x\n",g_BatteryNotifyCode);
			printk(" weak_charger = %d\n",weak_charger);
		}
		else
		{
			//g_BatteryNotifyCode &= ~(0x0008);
	...
	//battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,CHARGE_CURRENT_1250_00_MA);
	fan5405_reg_config_interface(0x04,0x7a);
	}
}
...
static void mt_battery_CheckBatteryStatus(void)
{
    ...
    if(mt_battery_CheckBatteryTemp() != PMU_STATUS_OK)
    {
	 ...
	 /*quanfang.wu add for FR572105 2014.02.11 begin*/	
...
#endif
 /*quanfang.wu add for FR572105 2014.02.11 end*/  

/*[FR565400][IEEE1725 requirement] add by [wangxingxing] at [20140213] start*/
/*[FR572105][Disable temp check and battary id check in MINI] add by [quanfang.wu] at [2014.02.13] begin*/
#if BATT_ID_CHECK_SUPPORT
#ifdef TARGET_BUILD_MMITEST	
	if(!( (get_boot_mode()==META_BOOT) || (get_boot_mode()==ADVMETA_BOOT) || (get_boot_mode()==ATE_FACTORY_BOOT) ))
 	{	
...
#endif
#endif
/*[FR572105][Disable temp check and battary id check in MINI] add by [quanfang.wu] at [2014.02.13] end*/
/*[FR565400][IEEE1725 requirement] add by [wangxingxing] at [20140213] end*/
...
static void mt_battery_notify_weak_charger_check(void)
{
	if(weak_charger == 1)
	{
	    g_BatteryNotifyCode |= 0x0020;
...
/*quanfang.wu add for FR572105 2014.02.11 end*/
		...
		mt_battery_notify_weak_charger_check();
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else    
		if(BMT_status.temperature >= 60 || (BMT_status.temperature <= -20))// JRD BSP +++ "[eos-3G-kk]shut off phone if current temperature less than -20"
        {
            #if defined(CONFIG_POWER_EXT)
            ...
            #else
            {
                ...
                else
                {
                    ...
                    battery_xlog_printk(BAT_LOG_CRTI, "[Battery] Tbat(%d)>=60|| <=-20, system need power down.\n", BMT_status.temperature);
...
{
	printk("[do_chrdet_int_task] g_bat_init_flag == %d\n",g_bat_init_flag);
    if((g_bat_init_flag == KAL_TRUE) && (g_thread_init_flag == KAL_TRUE))
    {
        ...
        else
        {
            ...
            if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
            {
    			...
    			printk("[do_chrdet_int_task] kernel charging boot ");
				msleep(3000);
				if(upmu_is_chr_det() == KAL_TRUE)
				{
					return;
...
/*#ifdef MTK_KERNEL_POWER_OFF_CHARGING
	 ...
	 {
		 ktime = ktime_set(3, 0);  // 10s, 10* 1000 ms
	 }
#else*/
       ktime = ktime_set(3, 0);  // 10s, 10* 1000 ms
//#endif
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TURN_OFF_CHARGING_FOR_FACTORY:
            /*  Disable charger */
           g_factory_charging = 1;
	    printk("turn_off_charging for factory\n");
            break;
	case TURN_ON_CHARGING_FOR_FACTORY:
            /*  Enable charger */
           g_factory_charging = 0;
	    printk("turn_on_charging for factory\n");
            break;
...
//Start
#ifdef EOS_3G_CHARGING_TEMPERATURE_PLOICY
		 tpd_enable_change_scanning_frq_();
		
        }else{
		 tpd_disable_change_scanning_frq_();
#endif		 
	 }
//End
