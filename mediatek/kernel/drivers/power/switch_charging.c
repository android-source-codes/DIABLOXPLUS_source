...
extern int Stop_Charging_Above_4095;
#endif
/*JRD BSP END*/
...
#endif
extern int max_charge_tempertaure ;
 // ============================================================ //
 //global variable
 // ============================================================ //
...
static void pchr_turn_on_charging (void)
{
 ...
 #if defined(DIABLOXPLUS_CHARGING_TEMPERATURE_PLOICY)
	if(Stop_Charging_Above_4095 == 1)
		{
			charging_enable = KAL_FALSE;
			printk("JRD stop charging when temperature above 45 and voltage above 4.1V \n");
		}
#endif
...
PMU_STATUS BAT_ConstantCurrentModeAction(void)
{
	...
	if((charging_full_check() == KAL_TRUE) && (BMT_status.bat_vol > 4200))
	{
	    BMT_status.bat_charging_state = CHR_BATFULL;
		BMT_status.bat_full = KAL_TRUE;
		g_charging_full_reset_bat_meter = KAL_TRUE;
...
PMU_STATUS BAT_CHR_TOP_OFF_Action(void)
{
	printk("JRD BMT_status.TOPOFF_charging_time=%d\n",BMT_status.TOPOFF_charging_time);
	//Sync UI full from 99% to 100%
	if((BMT_status.TOPOFF_charging_time > 15*60) && (BMT_status.bat_charging_state == CHR_TOP_OFF))
	{
	     BMT_status.bat_full = KAL_FALSE;
            BMT_status.bat_in_recharging_state = KAL_TRUE;
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[BATTERY:fan5405] Battery sync UI full from 99 to 100%.\n");
	}
	    BMT_status.PRE_charging_time = 0;
	    BMT_status.total_charging_time += BAT_TASK_PERIOD;
		 /*  Enable charger */
