...
void chrdet_int_handler(void)
{
    ...
    printk("[Power&PMIC] chrdet_int_handler start !\n");
    ...
    if (!upmu_get_rgs_chrdet())
    {
		...
		//JRD BSP START
		//solution of CDP reboot.
		printk("[Power&PMIC]power off charging boot ");
		msleep(3000);
		if(upmu_get_rgs_chrdet())
		{
			return;
		}
		printk("[Power&PMIC] !upmu_get_rgs_chrdet() true!\n");
		//JRD BSP END
		chr_control_interface(CHARGING_CMD_ENABLE, &disable);
		mt_power_off();
        }
    }
#endif
