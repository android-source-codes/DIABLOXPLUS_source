...
static int get_accdet_status()
{
	...
	if(iphone_headset == 1){
		mt_set_gpio_out(GPIO117, GPIO_OUT_ZERO); 
	}
	 headset_id_vol = get_headset_adc();
	 if(800 < headset_id_vol && headset_id_vol < 1200){	 	
		iphone_headset = 1;
	 }
	ACCDET_DEBUG("[Accdet] gyc  GPIO117 = %d,iphone_headset = %d\n",mt_get_gpio_out(GPIO117),iphone_headset);
	current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6);
	if(iphone_headset == 1)
		mt_set_gpio_out(GPIO117, GPIO_OUT_ONE);
	ACCDET_DEBUG("[Accdet] gyc  GPIO117 = %d,iphone_headset = %d\n",mt_get_gpio_out(GPIO117),iphone_headset);
...
//Disable ACCDET
		...
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		  ...
		  cable_pin_recognition = 0;
    		...
    		mt_set_gpio_out(GPIO117, GPIO_OUT_ZERO); 
		iphone_headset = 0;
		ACCDET_DEBUG("[Accdet] gyc GPIO_OUT_ZERO GPIO117 = %d,iphone_headset = %d\n",mt_get_gpio_out(GPIO117),iphone_headset);
    }
    //unmask EINT
    //msleep(500);
...
#endif
	mt_set_gpio_mode(GPIO117, GPIO_MODE_00);
	mt_set_gpio_pull_enable(GPIO117, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO117, GPIO_PULL_UP);
	mt_set_gpio_dir(GPIO117, GPIO_DIR_OUT);
}
/*-----------------------------------sysfs-----------------------------------------*/
#if DEBUG_THREAD
...
static ssize_t show_headset_vol(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	int headset_id_vol = 0;
	   ...
	   headset_id_vol = data[0]*1000+data[1]*10;
	   printk("accdet  adc: channel1  headset_id_vol =%d     GPIO117 = %d\n",headset_id_vol,mt_get_gpio_out(GPIO117));
