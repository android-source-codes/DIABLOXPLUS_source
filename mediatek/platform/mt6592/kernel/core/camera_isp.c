...
#include <mach/mt_cpufreq.h>    //For 92 freq over driver to 400MHz
...
//#define OVERRUN_AEE_WARNING   // mark for mt6592_wuxga (need to check sensor overrun for long team issue advence)
...
//Sensor IF
//Sensor IF
#define SENSOR_BASEADDR                 (CAMINF_BASE + 0x8000)
#define SENSORINF_INTEN                 (SENSOR_BASEADDR + 0x14)
#define SENSORINF_INTSTA                (SENSOR_BASEADDR + 0x18)
#define SENSORINF_CSI2_INTEN            (SENSOR_BASEADDR + 0x108)
#define SENSORINF_CSI2_INTSTA           (SENSOR_BASEADDR + 0x10C)
#define SENSORINF_NCSI2_INTEN           (SENSOR_BASEADDR + 0x614)
#define SENSORINF_NCSI2_INTSTA          (SENSOR_BASEADDR + 0x618)
...
//====== SENSOR IF IRQ======
#define SENINF1_INT_STATUS                 (SENIF_OVERRUN_IRQ_EN |\
                                            SENIF_CRCERR_IRQ |\
                                            SENIF_FSMERR_IRQ_EN |\
                                            SENIF_VSIZEERR_IRQ_EN |\
                                            SENIF_HSIZEERR_IRQ_EN |\
                                            SENIF_SENSOR_VSIZEERR_IRQ_EN |\
                                            SENIF_SENSOR_HSIZEERR_IRQ_EN)

#define SENINF1_CSI2_INT_STATUS            (CSI_CRC_ERR_IRQ |\
                                            CSI_ECC_ERR_IRQ |\
                                            CSI_ECC_CORRECT_IRQ |\
...
#define SENINF1_NCSI2_INT_STATUS            (NCSI_ERR_FRAME_SYNC |\
                                            ...
                                            NCSI_ERR_ECC_CORRECTED |\
                                            NCSI_ERR_ECC_DOUBLE |\
                                            NCSI_ERR_CRC |\
                                            NCSI_ERR_AFIFO |\
                                            NCSI_ERR_MULTI_LANE_SYNC |\
                                            NCSI_ERR_SOT_SYNC_HS_LNRD0 |\
                                            NCSI_ERR_SOT_SYNC_HS_LNRD1 |\
                                            NCSI_ERR_SOT_SYNC_HS_LNRD2 |\
                                            NCSI_ERR_SOT_SYNC_HS_LNRD3 |\
                                            NCSI_FS |\
                                            NCSI_LS |\
                                            NCSI_GS )
#endif
...
//Sensor INF IRQ Spinlock
spinlock_t  gSpinLockSenIRQ;
...
static MINT32 ISP_DumpReg(MVOID)
{
    ...
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4110, ISP_RD32((void *)(ISP_ADDR + 0x4110)));    
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4114, ISP_RD32((void *)(ISP_ADDR + 0x4114)));        
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4118, ISP_RD32((void *)(ISP_ADDR + 0x4118)));    
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x411C, ISP_RD32((void *)(ISP_ADDR + 0x411C)));        
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4120, ISP_RD32((void *)(ISP_ADDR + 0x4120)));
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4124, ISP_RD32((void *)(ISP_ADDR + 0x4124)));        
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4128, ISP_RD32((void *)(ISP_ADDR + 0x4128)));        
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x412C, ISP_RD32((void *)(ISP_ADDR + 0x412C)));
    LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4130, ISP_RD32((void *)(ISP_ADDR + 0x4130)));        
...
static MVOID ISP_EnableClock(MBOOL En)
{
    ...
    if(En)  // enable clock.
    {
        enable_clock(MT_CG_DISP0_SMI_COMMON,"CAMERA"); //new adder, confirm with Abrams
...
//            LOG_DBG(" <");
    ...
    else    // disable clock.
    {
        ...
        disable_clock(MT_CG_DISP0_SMI_COMMON,"CAMERA"); //new confirm with Abrams
        g_EnableClkCnt--;
...
static MINT32 ISP_RTBC_ENQUE(MINT32 dma)
{
    ...
    #if 0   // for HDR and camera lunch time optimization
                    ...
                    for(i = 0; i < g_deque_buf.count; i++)
                    {
                        ...
                        //{
                            LOG_DBG("[rtbc][DEQUE]:TStamp\"%d.%06d\",co(0x%08x),c2o(0x%08x),i(%d),iBuf+i(%d),VA(0x%x),PA(0x%x),O(0x%x),2O(0x%x)", \
                                ...
                                ISP_RD32((void *)ISP_REG_ADDR_IMG2O_BASE_ADDR));
                        //}
                        //CHeck HDR abnormal image fixed or not.
                        if(1)
                        {
                            if(imgo_fbc.Bits.FB_NUM ==2)
                            {
                                MUINT32 l32IMGO_ADDR = ISP_RD32((void *)ISP_REG_ADDR_IMGO_BASE_ADDR);
                                if((l32IMGO_ADDR!=g_pstRTBuf->ring_buf[rt_dma].data[0].base_pAddr)&&(l32IMGO_ADDR!=g_pstRTBuf->ring_buf[rt_dma].data[1].base_pAddr))
                                {
                                    LOG_DBG("[rtbc][DEQUE]:[0] = (0x%08x) 0",g_pstRTBuf->ring_buf[rt_dma].data[0].base_pAddr);
                                    LOG_DBG("[rtbc][DEQUE]:[1] = (0x%08x) 1",g_pstRTBuf->ring_buf[rt_dma].data[1].base_pAddr);                                 	

                                    LOG_DBG("IMGO_ADDR is OUT OF ring buff list table  (0x%08x) 0 1",l32IMGO_ADDR);                                 
//                                    aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT, "CAMERA", "IMGO_ADDR is OUT OF any buff list table");
//                                    LOG_DBG("aee_kernel_warning_api ");     
                                }
                            }
                        }
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_TG1_ERR_ST)
    {
        ...
        if(senifIntStA & 0x01)
        {
            ...
            if(regVal & 0x01)
            {
                ...
                LOG_DBG("43a8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3a8)));
                LOG_DBG("43b0(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3b0)));
                LOG_DBG("43b8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3b8)));
                LOG_DBG("43c4(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3c4)));
                LOG_DBG("43c8(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3c8)));
                LOG_DBG("43cc(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3cc)));
                LOG_DBG("43d0(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3d0)));
                LOG_DBG("43d4(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x3d4)));
                LOG_DBG("4040(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x040)));
                LOG_DBG("4044(0x%08x)",ISP_RD32((void *)(ISP_ADDR + 0x044)));
...
#ifdef SENSORINF_IRQ
static __tcmfunc irqreturn_t SENSORIF_Irq(MINT32  Irq, MVOID *DeviceId)
{
    MUINT32 IrqStatus[3];

    // Read irq status(Read clear)
    IrqStatus[0]  = ISP_RD32((void *)SENSORINF_INTSTA);
    IrqStatus[1]  = ISP_RD32((void *)SENSORINF_CSI2_INTSTA);
    IrqStatus[2]  = ISP_RD32((void *)SENSORINF_NCSI2_INTSTA);
    
    //spin_lock(&(ggSpinLockSenIRQ));
    //LOG_DBG("SENINF ENA IN(0x%08x),CSI2(0x%08x), NCSI2(0x%08x)",ISP_RD32((void *)SENSORINF_INTEN),ISP_RD32((void *)SENSORINF_CSI2_INTEN),ISP_RD32((void *)SENSORINF_NCSI2_INTEN));
    LOG_DBG("SENINF IRQ IN(0x%08x),CSI2(0x%08x), NCSI2(0x%08x)",IrqStatus[0],IrqStatus[1],IrqStatus[2]);
    //spin_unlock(&(gSpinLockSenIRQ));
...
static long ISP_ioctl(struct file *pFile,MUINT32 Cmd,unsigned long Param)
{
    ...
    MUINT32 lu32IspClkOn = 0; //For ISP CLOCK Overdrive enable
    ...
    case ISP_WAKELOCK_CTRL:
        ...
        case ISP_REF_CNT_CTRL:
            ...
            break;
        case ISP_CLK_OD_EN: //For ISP CLOCK Overdrive enable
        {
            if(copy_from_user(&lu32IspClkOn, (void*)Param, sizeof(MUINT32)) == 0)
            {
                LOG_DBG("[ISP_CLK_OD_EN]:0x08%x ",lu32IspClkOn);
                if(lu32IspClkOn==0)
                {
                    mt_soc_dvfs(SOC_DVFS_TYPE_ZSD, 0); //For ISP CLOCK Overdrive enable
                    LOG_INF("mt_soc_dvfs(SOC_DVFS_TYPE_ZSD, 0)");    
                }
                else
                {
                    mt_soc_dvfs(SOC_DVFS_TYPE_ZSD, 1); //For ISP CLOCK Overdrive enable
                    LOG_INF("mt_soc_dvfs(SOC_DVFS_TYPE_ZSD, 1)"); 
                }
            }
            else
            {
...
#ifdef SENSORINF_IRQ
    if (request_irq(SENINF_IRQ_ID, (irq_handler_t)SENSORIF_Irq, IRQF_TRIGGER_LOW , "sensor", NULL))
    {
       LOG_ERR("SENIF_IRQ_LINE IRQ LINE NOT AVAILABLE!!");
        goto EXIT;
    }
#endif
...
#ifdef SENSORINF_IRQ
    // Release Sensor IF IRQ 
    free_irq(SENINF_IRQ_ID , NULL);
#endif     
