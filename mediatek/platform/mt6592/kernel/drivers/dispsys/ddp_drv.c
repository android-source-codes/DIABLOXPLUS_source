...
#define DISP_INDEX_OFFSET 0xA000 // must be consistent with ddp_rdma.c
...
static /*__tcmfunc*/ irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
    ...
    switch(irq)
    {
        ...
        case MT_DISP_OVL_IRQ_ID:
                ...
                if(reg_val&(1<<5))
                {
                    if (cnt_ovl_abnormal % 256 == 0) {
                		DISP_ERR("IRQ: OVL-L0 not complete untill EOF! cnt=%d\n", cnt_ovl_abnormal++);
                		disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
                        disp_irq_log_module |= (1<<DISP_MODULE_RDMA);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
                ...
                {
                    if (cnt_ovl_abnormal % 256 == 0) {
                		DISP_ERR("IRQ: OVL-L1 not complete untill EOF! cnt=%d\n", cnt_ovl_abnormal++);
                		disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
                        disp_irq_log_module |= (1<<DISP_MODULE_RDMA);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
                ...
                {
                    if (cnt_ovl_abnormal % 256 == 0) {
                		DISP_ERR("IRQ: OVL-L2 not complete untill EOF! cnt=%d\n", cnt_ovl_abnormal++);
                		disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
                        disp_irq_log_module |= (1<<DISP_MODULE_RDMA);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
                ...
                {
                    if (cnt_ovl_abnormal % 256 == 0) {
                		DISP_ERR("IRQ: OVL-L3 not complete untill EOF! cnt=%d\n", cnt_ovl_abnormal++);
                		disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
                        disp_irq_log_module |= (1<<DISP_MODULE_RDMA);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
        ...
        case MT_DISP_WDMA_IRQ_ID:
                ...
                if(reg_val&(1<<1))
                {
                	if (cnt_wdma_underflow% 256 == 0) {
                        DISP_ERR("IRQ: wdma0 underrun! cnt=%d \n", cnt_wdma_underflow++);
                        disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
...
//	                      }
                ...
                if(reg_val&(1<<4))
                {
                	if (cnt_rdma_underflow % 256 == 0) {
                        DISP_ERR("IRQ: RDMA underflow! cnt=%d \n", cnt_rdma_underflow++);
                        disp_check_clock_tree();
                        disp_irq_log_module |= (1<<DISP_MODULE_OVL);
                        disp_irq_log_module |= (1<<DISP_MODULE_RDMA);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
...
//            disp_invoke_irq_callbacks(DISP_MODULE_COLOR, reg_val);
        ...
        case MT_DISP_MUTEX_IRQ_ID:  // can not do reg update done status after release mutex(for ECO requirement),
            ...
            if(reg_val & 0x0FF00) // udpate timeout intr triggered
            {
                ...
                for(mutexID=0;mutexID<4;mutexID++)
                {
                    if((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA) & (1<<(mutexID+9))) == (1<<(mutexID+9)))
                    {
                        DISP_ERR("disp_path_release_mutex() %d timeout! \n",mutexID);
                        disp_irq_log_module |= (1<<DISP_MODULE_CONFIG);
                        disp_irq_log_module |= (1<<DISP_MODULE_MUTEX);
                        disp_dump_reg(DISP_MODULE_CONFIG);
                        disp_dump_reg(DISP_MODULE_MUTEX);
                        ...
                        disp_print_reg_driectly(DISP_MODULE_MUTEX);
                        DISP_REG_SET(DISP_REG_CONFIG_MUTEX_RST(mutexID), 0);
                        DISP_MSG("mutex %d reset done! \n",mutexID);
                        disp_print_reg_driectly(DISP_MODULE_MUTEX);
                    }
                 }
            }
...
static int disp_probe(struct platform_device *pdev)
{
    ...
    DISP_REGISTER_IRQ(DISP_RDMA1_IRQ_ID);
    DISP_REGISTER_IRQ(MT_DISP_CMDQ_IRQ_ID);
    DISP_REGISTER_IRQ(MT_DISP_MUTEX_IRQ_ID);
    ...
    // set high freq if FHD or higher resolution
//    if(1)  //fix me by query resolution
...
static int disp_remove(struct platform_device *pdev)
{
    ...
    disable_irq(DISP_RDMA1_IRQ_ID);
    disable_irq(MT_DISP_CMDQ_IRQ_ID);
...
int disp_dump_reg(DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_WDMA0:
		...
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C00);
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C02);
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C10);
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C12);
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C20);
		g_reg_wdma[i++] = DISP_REG_GET(DISP_REG_WDMA_C22);
	...
	case DISP_MODULE_RDMA1:
		...
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_00				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_01				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_02				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_10				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_11				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_12				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_20				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_21				+ index*DISP_INDEX_OFFSET);
		g_reg_rdma[i++] = DISP_REG_GET(DISP_REG_RDMA_CF_22				+ index*DISP_INDEX_OFFSET);
...
int disp_print_reg_driectly(DISP_MODULE_ENUM module)
{
    int index = 0;
	switch(module)
	{
		case DISP_MODULE_OVL:
		DISP_MSG("===== DISP OVL Reg DIRECTLY Dump : ============\n");
		...
		DISP_MSG("===== DISP RDMA%d Reg DIRECTLY Dump: ======== \n", index);
		DISP_MSG("(00)R_INT_ENABLE   =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_INT_ENABLE         + index*DISP_INDEX_OFFSET));
		DISP_MSG("(04)R_INT_STATUS   =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_INT_STATUS         + index*DISP_INDEX_OFFSET));
		DISP_MSG("(10)R_GLOBAL_CON   =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON         + index*DISP_INDEX_OFFSET));
		DISP_MSG("(14)R_SIZE_CON_0   =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0         + index*DISP_INDEX_OFFSET));
		DISP_MSG("(18)R_SIZE_CON_1   =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1	      + index*DISP_INDEX_OFFSET));
		DISP_MSG("(1C)R_TARGET_LINE  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_TARGET_LINE        + index*DISP_INDEX_OFFSET));
		DISP_MSG("(24)R_MEM_CON      =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_CON 		      + index*DISP_INDEX_OFFSET));
		DISP_MSG("(28)R_MEM_START_ADDR    =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_START_ADDR	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(2C)R_MEM_SRC_PITCH     =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_SRC_PITCH 	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(30)R_MEM_GMC_SETTING_0 =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_GMC_SETTING_0	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(34)R_MEM_SLOW_CON      =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_SLOW_CON	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(30)R_MEM_GMC_SETTING_1 =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_MEM_GMC_SETTING_1 	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(40)R_FIFO_CON          =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_FIFO_CON		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(54)R_CF_00  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_00		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(58)R_CF_01  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_01		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(5C)R_CF_02  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_02		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(60)R_CF_10  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_10		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(64)R_CF_11  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_11		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(68)R_CF_12  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_12		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(6C)R_CF_20  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_20		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(70)R_CF_21  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_21		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(74)R_CF_22  =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_22		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(78)R_CF_PRE_ADD0       =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_PRE_ADD0	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(7C)R_CF_PRE_ADD1       =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_PRE_ADD1	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(80)R_CF_PRE_ADD2       =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_PRE_ADD2	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(84)R_CF_POST_ADD0      =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_POST_ADD0	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(88)R_CF_POST_ADD1      =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_POST_ADD1	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(8C)R_CF_POST_ADD2      =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_CF_POST_ADD2	+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(90)R_DUMMY             =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_DUMMY		+ index*DISP_INDEX_OFFSET));
		DISP_MSG("(94)R_DEBUG_OUT_SEL     =0x%x \n", DISP_REG_GET(DISP_REG_RDMA_DEBUG_OUT_SEL	+ index*DISP_INDEX_OFFSET));
		break;        
	    case DISP_MODULE_MUTEX:
	    DISP_MSG("===== DISP DISP_REG_MUTEX Reg Dump DIRECTLY: ============\n");
		DISP_MSG("(0 )M_INTEN        =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTEN));
		DISP_MSG("(4 )M_INTSTA       =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX_INTSTA));
		DISP_MSG("(8 )REG_UPD_TIMEOUT    =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_REG_UPD_TIMEOUT));
		DISP_MSG("(C )REG_COMMIT         =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_REG_COMMIT));
		DISP_MSG("(20)M0_EN   =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_EN));
		DISP_MSG("(24)M0      =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX0));
		DISP_MSG("(28)M0_RST  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_RST));
		DISP_MSG("(2C)M0_MOD  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_MOD));
		DISP_MSG("(30)M0_SOF  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX0_SOF));
		DISP_MSG("(40)M1_EN   =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_EN));
		DISP_MSG("(44)M1      =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX1));
		DISP_MSG("(48)M1_RST  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_RST));
		DISP_MSG("(4C)M1_MOD  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_MOD));
		DISP_MSG("(50)M1_SOF  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX1_SOF));
		DISP_MSG("(60)M2_EN   =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_EN));
		DISP_MSG("(64)M2      =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX2));
		DISP_MSG("(68)M2_RST  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_RST));
		DISP_MSG("(6C)M2_MOD  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_MOD));
		DISP_MSG("(70)M2_SOF  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX2_SOF));
		DISP_MSG("(80)M3_EN   =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_EN));
		DISP_MSG("(84)M3      =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX3));
		DISP_MSG("(88)M3_RST  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_RST));
		DISP_MSG("(8C)M3_MOD  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_MOD));
		DISP_MSG("(90)M3_SOF  =0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX3_SOF));
		DISP_MSG("(100)M_DEBUG_OUT_SEL=0x%x \n", DISP_REG_GET(DISP_REG_CONFIG_MUTEX_DEBUG_OUT_SEL));
		break;
	default:
		DISP_MSG("error, reg_dump_directly, unknow module=%d \n", module);
		break;
	}
	return 0;
}
...
void disp_print_reg(DISP_MODULE_ENUM module)
{
	...
	case DISP_MODULE_COLOR:
		...
		DISP_MSG("(F00)C_START     =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F04)C_INTEN     =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F08)C_INTSTA    =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F0C)C_OUT_SEL   =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F10)C_FRAME_DONE_DEL    =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F14)C_CRC               =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F18)C_SW_SCRATCH        =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F20)C_RDY_SEL           =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F24)C_RDY_SEL_EN        =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F28)C_CK_ON             =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F50)C_INTERNAL_IP_WIDTH =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F54)C_INTERNAL_IP_HEIGHT=0x%x \n", g_reg_clr[i++]);
		DISP_MSG("(F60)C_CM1_EN            =0x%x \n", g_reg_clr[i++]);
		DISP_MSG("TOTAL dump %d registers\n", i);
		break;

	case DISP_MODULE_BLS:
		DISP_MSG("===== DISP BLS Reg Dump: ============\n");
		DISP_MSG("(0 )B_EN      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(4 )B_RST     =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(8 )B_INTEN   =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(C )B_INTSTA  =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(10)B_BLS_SETTING       =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(14)B_FANA_SETTING      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(18)B_SRC_SIZE          =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(20)B_GAIN_SETTING      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(24)B_MANUAL_GAIN       =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(28)B_MANUAL_MAXCLR     =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(30)B_GAMMA_SETTING     =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(34)B_GAMMA_BOUNDARY    =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(38)B_LUT_UPDATE        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(60)B_MAXCLR_THD        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(64)B_DISTPT_THD        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(68)B_MAXCLR_LIMIT      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(6C)B_DISTPT_LIMIT      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(70)B_AVE_SETTING       =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(74)B_AVE_LIMIT         =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(78)B_DISTPT_SETTING    =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(7C)B_HIS_CLEAR         =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(80)B_SC_DIFF_THD       =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(84)B_SC_BIN_THD        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(88)B_MAXCLR_GRADUAL    =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(8C)B_DISTPT_GRADUAL    =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(90)B_FAST_IIR_XCOEFF   =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(94)B_FAST_IIR_YCOEFF   =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(98)B_SLOW_IIR_XCOEFF   =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(9C)B_SLOW_IIR_YCOEFF   =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(A0)B_PWM_DUTY          =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(A4)B_PWM_GRADUAL       =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(A8)B_PWM_CON           =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(AC)B_PWM_MANUAL        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(B0)B_DEBUG             =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(B4)B_PATTERN           =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(B8)B_CHKSUM            =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(200)B_PWM_DUTY_RD      =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(204)B_FRAME_AVE_RD     =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(208)B_MAXCLR_RD        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(20C)B_DISTPT_RD        =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(210)B_GAIN_RD          =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("(214)B_SC_RD            =0x%x \n", g_reg_bls[i++]);
		DISP_MSG("TOTAL dump %d registers\n", i);
		break;
	...
	case DISP_MODULE_DSI_VDO:
	case DISP_MODULE_DSI_CMD:
	    DISP_MSG("===== DISP DSI Reg Dump: ============\n");
	    DSI_DumpRegisters();
		break;

	case DISP_MODULE_CONFIG:
		DISP_MSG("===== DISP DISP_REG_MM_CONFIG Reg Dump: ============\n");
		DISP_MSG("(1c)CAM_MDP_MOUT_EN   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(20)MDP_RDMA_MOUT_EN  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(24)MDP_RSZ0_MOUT_EN  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(28)MDP_RSZ1_MOUT_EN  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(2c)MDP_TDSHP_MOUT_EN =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(30)DISP_OVL_MOUT_EN  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(34)MMSYS_MOUT_RST    =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(38)MDP_RSZ0_SEL  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(3c)MDP_RSZ1_SEL  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(40)MDP_TDSHP_SEL =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(44)MDP_WROT_SEL  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(48)MDP_WDMA_SEL  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(4c)DISP_OUT_SEL  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(100)CG_CON0  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(104)CG_SET0  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(108)CG_CLR0  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(110)CG_CON1  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(114)CG_SET1  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(118)CG_CLR1  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(120)HW_DCM_DIS0       =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(124)HW_DCM_DIS_SET0   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(128)HW_DCM_DIS_CLR0   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(12c)HW_DCM_DIS1       =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(130)HW_DCM_DIS_SET1   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(134)HW_DCM_DIS_CLR1   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(138)SW_RST_B    =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(13c)LCM_RST_B   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(800)MBIST_DONE  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(804)MBIST_FAIL0 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(808)MBIST_FAIL1 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(80C)MBIST_HOLDB =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(810)MBIST_MODE  =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(814)MBIST_BSEL0 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(818)MBIST_BSEL1 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(81c)MBIST_CON   =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(820)MEM_DELSEL0 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(824)MEM_DELSEL1 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(828)MEM_DELSEL2 =0x%x \n", g_reg_cfg[i++]);
		DISP_MSG("(82c)MEM_DELSEL3 =0x%x \n", g_reg_cfg[i++]);
	...
	case DISP_MODULE_MUTEX:
		...
		DISP_MSG("(8C)M3_MOD  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(90)M3_SOF  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(A0)M4_EN   =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(A4)M4      =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(A8)M4_RST  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(AC)M4_MOD  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(B0)M4_SOF  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(C0)M5_EN   =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(C4)M5      =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(C8)M5_RST  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(CC)M5_MOD  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(D0)M5_SOF  =0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("(100)M_DEBUG_OUT_SEL=0x%x \n", g_reg_mtx[i++]);
		DISP_MSG("TOTAL dump %d registers\n", i);
		break;
