...
//#define EXT_CODEC //external codec mode

#define AUDDRV_DL1_MAX_BUFFER_LENGTH (0x4000)
#define MT6592_AFE_MCU_IRQ_LINE (136 + 32)
...
static DEFINE_SPINLOCK(auddrv_WriteRecord);
...
#if 1//Added by jrd.lipeng for hifi chip existence check
static bool ecodec_register_state = false; //false: failed; true: success
#endif
...
unsigned long long prev_write_tin = 0;
unsigned long long prev_write_tout = 0;



static kal_uint32 UL_Interrupt_Interval = 50;   //default align AFE_UL_TIMEOUT time
...
static DEFINE_MUTEX(gextcodec_mutex);
#endif
...
extern int        Aud_AFE_Clk_cntr;
extern int        Aud_LJ22M_Clk_cntr;
extern int        Aud_LJ24M_Clk_cntr;
...
static kal_uint32 inline AudDrv_SampleRateIndexConvert(kal_uint32 SampleRateIndex)
{
...
static void inline AudDrv_getDLInterval(void)
{
...
static void inline AudDrv_getULInterval(void)
{
    kal_uint32 samplerate = Afe_Get_Reg(AFE_IRQ_MCU_CON);
    kal_uint32 InterruptSample = Afe_Get_Reg(AFE_IRQ_CNT2);
    samplerate = (samplerate >> 8) & 0x0000000f;
    samplerate = AudDrv_SampleRateIndexConvert(samplerate);
    UL_Interrupt_Interval = ((InterruptSample * 1000) / samplerate) + 1;
    isEchoRefModeChange = false;
    PRINTK_AUDDRV("UL_Interrupt_Interval = %d\n",UL_Interrupt_Interval);
}
...
void SaveWriteWaitEvent(unsigned int  t2)
{
    ...
    spin_unlock(&auddrv_WriteRecord);
}
...
void ResetWriteWaitEvent_l(void)
{
    int i = 0;
    for (i = 0; i < WriteArrayMax ; i++)
    {
...
static int AudDrv_Read_Procmem(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{
    ...
    len += sprintf(buf + len , "Aud_LJ22M_Clk_cntr = 0x%x\n", Aud_LJ22M_Clk_cntr);
    len += sprintf(buf + len , "Aud_LJ24M_Clk_cntr = 0x%x\n", Aud_LJ24M_Clk_cntr);
    ...
    len += sprintf(buf + len , "AFE_CONN_24BIT  = 0x%x\n", Afe_Get_Reg(AFE_CONN_24BIT));
    ...
    len += sprintf(buf + len , "AFE_DAI_BASE  = 0x%x\n", Afe_Get_Reg(AFE_DAI_BASE));
    len += sprintf(buf + len , "AFE_DAI_END  = 0x%x\n", Afe_Get_Reg(AFE_DAI_END));
...
static int AudDrv_Read_Proc_ExtCodec(char *buf, char **start, off_t offset, int count , int *eof, void *data)
{
    int len = 0;
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
#if 0
    len += sprintf(buf + len , "CS4398_MODE = 0x%x\n", ExtCodec_ReadReg(CS4398_MODE));
    len += sprintf(buf + len , "CS4398_MIXING = 0x%x\n", ExtCodec_ReadReg(CS4398_MIXING));
    len += sprintf(buf + len , "CS4398_MUTE = 0x%x\n", ExtCodec_ReadReg(CS4398_MUTE));
    len += sprintf(buf + len , "CS4398_VOLA = 0x%x\n", ExtCodec_ReadReg(CS4398_VOLA));
    len += sprintf(buf + len , "CS4398_VOLB = 0x%x\n", ExtCodec_ReadReg(CS4398_VOLB));
    len += sprintf(buf + len , "CS4398_RAMP = 0x%x\n", ExtCodec_ReadReg(CS4398_RAMP));
    len += sprintf(buf + len , "CS4398_MISC1 = 0x%x\n", ExtCodec_ReadReg(CS4398_MISC1));
    len += sprintf(buf + len , "CS4398_MISC2 = 0x%x\n", ExtCodec_ReadReg(CS4398_MISC2));
#else
    u8 i = 0;

    for(i = 0; i <= ES9018_REG_END; i++) {
        len += sprintf(buf + len , "ES9018_REG#%d = 0x%x\n", i, ExtCodec_ReadReg(i));
    }
#endif
#endif
    return len;
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT//Added by jrd.lipeng for smart pa & hifi debugging
static ssize_t show_hifi_chip_exist(struct device_driver *ddri, char *buf)
{
    bool stat = ecodec_register_state;
    return sprintf(buf, "%s\n", stat == true ? "yes" : "no");
}

static ssize_t show_hifi_reg(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    u8 i = 0;
...
static ssize_t store_hifi_reg(struct device_driver *ddri, const char *buf, size_t count)
{
    u8 reg, val;
    int ret = 0;

    sscanf(buf, "%u=%x", &reg, &val);
    PRINTK_AUDDRV("[%s] ExtCodec_WriteReg(%u, 0x%x)", __func__, reg, val);
    ret = ExtCodec_WriteReg(reg, val);
    if(ret < 0) {
        PRINTK_AUDDRV(KERN_ERR "[%s] ExtCodec_WriteReg(%u, %u) failed!", __func__, reg, val);
        return -1;
    }
    return count;
}

#define SWITCH2HIFI (GPIO_OUT_ZERO) //0 -> hifi; 1 -> pmic
static ssize_t show_hifi_pmic_switch(struct device_driver *ddri, char *buf)
{
    int val = 0;

    val = mt_get_gpio_out(GPIO_SWITCH1_1V8_PIN);
    return sprintf(buf, "%s\n", val == SWITCH2HIFI ? "HIFI" : "PMIC");
}

static ssize_t store_hifi_pmic_switch(struct device_driver *ddri, const char *buf, size_t count)
{
    unsigned int val;

    if (sscanf(buf, "%u", &val) != 1) 
    {
        PRINTK_AUDDRV(KERN_ERR "[%s]\n", __func__);
        return -EINVAL;
    }

    PRINTK_AUDDRV("[%s] change path to '%s'\n", __func__, val == SWITCH2HIFI ? "HIFI" : "PMIC");
    mt_set_gpio_out(GPIO_SWITCH1_1V8_PIN, val);
    return count;
}
#if 1//lipeng debug
void jrd_set_switch_delay(unsigned int ms);
static ssize_t store_hifi_switch_delay(struct device_driver *ddri, const char *buf, size_t count)
{
    unsigned int val;
    ...
    jrd_set_switch_delay(val);
    return count;
}
static DRIVER_ATTR(hifi_switch_delay, 0664, NULL, store_hifi_switch_delay);
#endif
static DRIVER_ATTR(hifi_chip_exist, 0664, show_hifi_chip_exist, NULL);
static DRIVER_ATTR(hifi_reg, 0664, show_hifi_reg, store_hifi_reg);
static DRIVER_ATTR(hifi_pmic_switch, 0664, show_hifi_pmic_switch, store_hifi_pmic_switch);

static struct driver_attribute *hifi_attr_list[] = {
#if 1//lipeng debug
    &driver_attr_hifi_switch_delay,
#endif
    //&driver_attr_hifi_reg,
    &driver_attr_hifi_pmic_switch,
    & driver_attr_hifi_chip_exist
};

static int hifi_create_attr(struct device_driver *driver) 
{
    int idx, err = 0;
    int num = (int)(sizeof(hifi_attr_list)/sizeof(hifi_attr_list[0]));
    if (driver == NULL) {
        return -EINVAL;
    }

    for(idx = 0; idx < num; idx++) {
        if((err = driver_create_file(driver, hifi_attr_list[idx]))) {
            PRINTK_AUDDRV("driver_create_file (%s) = %d\n", hifi_attr_list[idx]->attr.name, err);
            break;
        }
    }
    return err;
...
void Auddrv_DL_Interrupt_Handler(void)  // irq1 ISR handler
{
    ...
    PRINTK_AUDDRV("+Auddrv_DL_Interrupt_Handler ReadIdx:%x WriteIdx:%x, DataRemained:%x, Afe_consumed_bytes:%x HW_memory_index = %x \n",
        ...
        */
    PRINTK_DEBUG_MODE("+Auddrv debug_tool @@-1,%d,%d,%d,%d,%d,%d,0\n",
    ...
    if (Afe_Block->u4DataRemained < Afe_consumed_bytes || Afe_Block->u4DataRemained <= 0 || Afe_Block->u4DataRemained  > Afe_Block->u4BufferSize || AudIrqReset)
    {
        ...
        PRINTK_AUDDRV("AudDrv=>DataRemained:0x%x R:0x%x W:0x%x BufSize:0x%x Consumed_bytes:0x%x \n",Afe_Block->u4DataRemained,Afe_Block->u4DMAReadIdx,Afe_Block->u4WriteIdx,Afe_Block->u4BufferSize,Afe_consumed_bytes);
        PRINTK_AUDDRV("AudDrv=>HW_memory_index:0x%x pucPhysBufAddr:0x%x AudIrqReset:0x%x\n",HW_memory_index,Afe_Block->pucPhysBufAddr,AudIrqReset);
        ...
        if (isDebugMode)
        {
            //aee_kernel_exception_api(__FILE__, __LINE__, DB_OPT_FTRACE, "Audio buffer underflow, DEBUG_MODE", "audio blocked dump ftrace");
        }
        AudIrqReset = false;
        AudDebug_DLIrqUnderflow = true;
    }
    else
    {
        /*
...
#if 1//Modified by jrd.lipeng
    /* Becuase of hifi & p-sensor i2c address conflict problem, need load hifi driver
     * behind p-sensor driver. So I moved the loading codes of hifi driver to here, 
     * and control the sequence of hifi driver & p-senosr driver by the sequence of platform devices 
     * registration. */
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
    bool ret2 = false;
    //Register external DAC driver
    ret2 = ExtCodec_Register();
    ecodec_register_state = ret2;
    if(ret2 == false) {
        PRINTK_AUDDRV("ExtCodec_Register failed!\n");
    } else {
#if 0//Added by jrd.lipeng for new hifi ldo power timing sequence.
        /* To prevent the leakage of DVCC of ES9018, power on DVCC since boot-up. */
        mt_set_gpio_mode(GPIO_HIFI_DVCC_EN_PIN, GPIO_MODE_00);
        mt_set_gpio_out(GPIO_HIFI_DVCC_EN_PIN, GPIO_OUT_ONE);//DVCC, 1.8v
        msleep(1);
#if 1//Added by jrd.lipeng for new hifi ldo power timing sequence.
        /* Because hifi reset pin of PIO5 can't work, so move AVCC & VCCA to ES9018's driver. */
        /* enable AVCC & VCCA of ES9018. */
...
#endif
#endif
        ret = driver_create_file(dev->dev.driver, &driver_attr_hifi_reg);
        if(ret < 0) {
            PRINTK_AUDDRV("driver_create_file (%s) = %d\n", driver_attr_hifi_reg.attr.name, ret);
        }
    }
    hifi_create_attr(dev->dev.driver);
#endif
#endif

    PRINTK_AUDDRV("+request_irq \n");
    ret = request_irq(MT6592_AFE_MCU_IRQ_LINE, AudDrv_IRQ_handler, IRQF_TRIGGER_LOW/*IRQF_TRIGGER_FALLING*/, "Afe_ISR_Handle", dev);
    if (ret < 0)
    {
...
static void AudDrv_Store_reg_AFE(AudAfe_Suspend_Reg *pBackup_reg)
{
    ...
    pBackup_reg->Suspend_AFE_CONN_24BIT =             Afe_Get_Reg(AFE_CONN_24BIT);
    ...
    //pBackup_reg->Suspend_AFE_IRQ_CON=               Afe_Get_Reg(AFE_IRQ_CON);6-5-8-2
    ...
    //pBackup_reg->Suspend_AFE_MEMIF_MON3=            Afe_Get_Reg(AFE_MEMIF_MON3);6-5-8-2
    pBackup_reg->Suspend_AFE_MEMIF_MON4 =            Afe_Get_Reg(AFE_MEMIF_MON4);

    //pBackup_reg->Suspend_AFE_FOC_CON=               Afe_Get_Reg(AFE_FOC_CON);6-5-8-2
    //pBackup_reg->Suspend_AFE_FOC_CON1=              Afe_Get_Reg(AFE_FOC_CON1);6-5-8-2
    //pBackup_reg->Suspend_AFE_FOC_CON2=              Afe_Get_Reg(AFE_FOC_CON2);6-5-8-2
    //pBackup_reg->Suspend_AFE_FOC_CON3=              Afe_Get_Reg(AFE_FOC_CON3);6-5-8-2
    //pBackup_reg->Suspend_AFE_FOC_CON4=              Afe_Get_Reg(AFE_FOC_CON4);6-5-8-2
    //pBackup_reg->Suspend_AFE_FOC_CON5=              Afe_Get_Reg(AFE_FOC_CON5);6-5-8-2

    //pBackup_reg->Suspend_AFE_MON_STEP=              Afe_Get_Reg(AFE_MON_STEP);6-5-8-2
    ...
    //pBackup_reg->Suspend_AFE_MRG_MON0=              Afe_Get_Reg(AFE_MRG_MON0);6-5-8-2
    //pBackup_reg->Suspend_AFE_MRG_MON1=              Afe_Get_Reg(AFE_MRG_MON1);6-5-8-2
    //pBackup_reg->Suspend_AFE_MRG_MON2=              Afe_Get_Reg(AFE_MRG_MON2);6-5-8-2
    ...
    //pBackup_reg->Suspend_AFE_IRQ_MCU_CNT5=          Afe_Get_Reg(AFE_IRQ_CNT5);6-5-8-2
    ...
    Afe_Get_Reg(AFE_IRQ1_EN_CNT_MON);
    //pBackup_reg->Suspend_AFE_IRQ5_MCU_EN_CNT_MON;   Afe_Get_Reg(AFE_IRQ5_MCU_EN_CNT_MON);6-5-8-2
...
static void AudDrv_Recover_reg_AFE(AudAfe_Suspend_Reg *pBackup_reg)
{
    ...
    Afe_Set_Reg(AFE_CONN_24BIT,           pBackup_reg->Suspend_AFE_CONN_24BIT,          MASK_ALL);
    ...
    //Afe_Set_Reg(AFE_IRQ_CON,             pBackup_reg->Suspend_AFE_IRQ_CON,            MASK_ALL);6-5-8-2
    ...
    //Afe_Set_Reg(AFE_MEMIF_MON3,          pBackup_reg->Suspend_AFE_MEMIF_MON3,         MASK_ALL);6-5-8-2
    Afe_Set_Reg(AFE_MEMIF_MON4,          pBackup_reg->Suspend_AFE_MEMIF_MON4,         MASK_ALL);

    //Afe_Set_Reg(AFE_FOC_CON,             pBackup_reg->Suspend_AFE_FOC_CON,            MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_FOC_CON1,            pBackup_reg->Suspend_AFE_FOC_CON1,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_FOC_CON2,            pBackup_reg->Suspend_AFE_FOC_CON2,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_FOC_CON3,            pBackup_reg->Suspend_AFE_FOC_CON3,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_FOC_CON4,            pBackup_reg->Suspend_AFE_FOC_CON4,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_FOC_CON5,            pBackup_reg->Suspend_AFE_FOC_CON5,           MASK_ALL);6-5-8-2

    //Afe_Set_Reg(AFE_MON_STEP,            pBackup_reg->Suspend_AFE_MON_STEP,           MASK_ALL);6-5-8-2
    ...
    //Afe_Set_Reg(AFE_MRG_MON0,            pBackup_reg->Suspend_AFE_MRG_MON0,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_MRG_MON1,            pBackup_reg->Suspend_AFE_MRG_MON1,           MASK_ALL);6-5-8-2
    //Afe_Set_Reg(AFE_MRG_MON2,            pBackup_reg->Suspend_AFE_MRG_MON2,           MASK_ALL);6-5-8-2
    ...
    //Afe_Set_Reg(AFE_IRQ_CNT5,        pBackup_reg->Suspend_AFE_IRQ_MCU_CNT5,       MASK_ALL);6-5-8-2
    ...
    //Afe_Set_Reg(AFE_IRQ5_MCU_EN_CNT_MON, pBackup_reg->Suspend_AFE_IRQ5_MCU_EN_CNT_MON, MASK_ALL);6-5-8-2
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
#if 1//Added by jrd.lipeng for new hifi ldo power timing sequence.
        /* Power off DVCC when remove audio driver. */
        mt_set_gpio_mode(GPIO_HIFI_DVCC_EN_PIN, GPIO_MODE_00);
        mt_set_gpio_out(GPIO_HIFI_DVCC_EN_PIN, GPIO_OUT_ZERO);//DVCC, 1.8v
#if 1//Added by jrd.lipeng for new hifi ldo power timing sequence.
        /* Because hifi reset pin of PIO5 can't work, so move AVCC & VCCA to ES9018's driver. */
        /* Disable AVCC & VCCA of ES9018. */
        mt_set_gpio_mode(GPIO_HIFI_AVCC_EN_PIN, GPIO_MODE_00);
...
static int AudDrv_resume(struct platform_device *dev) // wake up
{
    ...
    clkmux_sel(MT_MUX_AUDINTBUS, 1, "AUDIO");
...
int AudDrv_Allocate_DL1_Buffer(struct file *fp, kal_uint32 Afe_Buf_Length)
{
    ...
    if (pblock->pucPhysBufAddr == 0)
    {
...
#endif
        //        Afe_Set_Reg(AFE_MEMIF_MAXLEN, 0, 0x01); //6-5-8-2
#else
        ...
        //        Afe_Set_Reg(AFE_MEMIF_MAXLEN, 1, 0x01); //6-5-8-2
#endif
    }
...
static long AudDrv_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SET_GPIO_CURRENT:
        {
            {
            	  //adjust GPIO 105~107 current to 2mA, 4mA, 6mA, 8mA
                //for 6592, bit[6:5] for GPIO 105 ~ 107, bit[2:1] for GPIO 17 ~ 20
                volatile uint32 address = GPIO_BASE + 0x0B54;
                volatile uint32 *ptr = (volatile uint32*)address;
                volatile uint32 flag,value;
                if(arg == 1)
                   flag = (1 << 5) | (1 << 1);
                else if (arg == 2)
                   flag = (2 << 5) | (2 << 1);
                ...
                if (flag)
                   mt65xx_reg_sync_writel(flag,ptr);
                msleep(1);
                address = GPIO_BASE + 0x0B50;
                ptr = (volatile uint32*)address;
                value = __raw_readl(ptr);
                PRINTK_AUDDRV("SET_GPIO_CURRENT flag = 0x%x, value1 = 0x%x,\n",flag, value);

                PRINTK_AUDDRV("SET_GPIO_CURRENT GPIO_normal after setting = %x, arg= %d,\n",value,arg);
        ...
        case AUD_RESTART:
        {
            ...
            else
            {
                ...
                if (Aud_LJ22M_Clk_cntr > 0)
                {
                    AudDrv_LowJitter22M_Clk_Off();
                    Aud_LJ22M_Clk_cntr = 0;
                }
                if (Aud_LJ24M_Clk_cntr > 0)
                {
                    AudDrv_LowJitter24M_Clk_Off();
                    Aud_LJ24M_Clk_cntr = 0;
                }
                Aud_HDMI_Clk_cntr = 0;
        ...
        case AUD_SET_CLOCK:
        {
            ...
            {
                spin_lock(&auddrv_lock);
                PRINTK_AUDDRV("AudDrv +Before AUD_SET_CLOCK lock\n");
                spin_lock_irqsave(&auddrv_clk_isr_lock, flags);
                PRINTK_AUDDRV("AudDrv +Before AUD_SET_CLOCK Func\n");
                AudDrv_Clk_On();
                PRINTK_AUDDRV("AudDrv -After AUD_SET_CLOCK Func\n");
                spin_unlock_irqrestore(&auddrv_clk_isr_lock, flags);
                PRINTK_AUDDRV("AudDrv After AUD_SET_CLOCK lock\n");
                spin_unlock(&auddrv_lock);
            }
            else
            {
        ...
        case SET_LOWJITTER_CLK_ON:
        {
            if (arg == 1)
            {
                //enable_mux(MT_MUX_AUD_24M, "name");
                PRINTK_AUDDRV("+AudDrv SET_LOWJITTER_CLK_ON(%ld) \n", arg);
                AudDrv_LowJitter24M_Clk_On();
            }
            else
            {
                //enable_mux(MT_MUX_AUD_22M, "name");
                PRINTK_AUDDRV("+AudDrv SET_LOWJITTER_CLK_ON(%ld) \n", arg);
                AudDrv_LowJitter22M_Clk_On();
            }
            break;
        }
        case SET_LOWJITTER_CLK_OFF:
        {
            if (arg == 1)
            {
                //disable_mux(MT_MUX_AUD_24M, "name");
                PRINTK_AUDDRV("+AudDrv SET_LOWJITTER_CLK_OFF(%ld) \n", arg);
                AudDrv_LowJitter24M_Clk_Off();
            }
            else
            {
                //disable_mux(MT_MUX_AUD_22M, "name");
                PRINTK_AUDDRV("+AudDrv SET_LOWJITTER_CLK_OFF(%ld) \n", arg);
                AudDrv_LowJitter22M_Clk_Off();
            }
            break;
...
#ifdef MTK_AUDIO_EXTCODEC_SUPPORT
        case SET_EXTCODEC_ON:
            mutex_lock(&gextcodec_mutex);
#if 1
            cust_extPLL_gpio_config(); //external PLL, set S0/S1
            //cust_extHPAmp_gpio_off(); //external HPAmp, AMP set to disable, gain set to 0dB
            //cust_extcodec_gpio_off(); //external DAC, RST set to LOW, DAC_PWREN = 0
#endif
            cust_extcodec_gpio_on(); //GPIO turn ON CS4398, RST change from Low to High
            ExtCodec_PowerOn();
            mutex_unlock(&gextcodec_mutex);
            break;
        case SET_EXTCODEC_OFF:
            mutex_lock(&gextcodec_mutex);
            //ExtCodec_PowerOff();
            cust_extcodec_gpio_off(); //GPIO turn OFF CS4398, RST change from High to Low
            mutex_unlock(&gextcodec_mutex);
            break;
        case SET_EXTHEADPHONE_AMP_ON:
            mutex_lock(&gextcodec_mutex);
            cust_extHPAmp_gpio_on(); //GPIO turn ON TPA6141, gain is set at 0 or +6dB by GPIO
            mutex_unlock(&gextcodec_mutex);
            break;
        case SET_EXTHEADPHONE_AMP_OFF:
            mutex_lock(&gextcodec_mutex);
            cust_extHPAmp_gpio_off(); //GPIO turn OFF TPA6141
            mutex_unlock(&gextcodec_mutex);
            break;
        case SET_EXTCODEC_MUTE:
            mutex_lock(&gextcodec_mutex);
            ExtCodec_Mute();
            mutex_unlock(&gextcodec_mutex);
            break;
        case SET_EXTCODEC_GAIN: //// 0x0000BBAA,  BB(00: left, 01: right), AA(volume set to DAC register)
        {
            int leftright = 0, gain;
            mutex_lock(&gextcodec_mutex);
            if ((arg & 0xFF00) == 0x0100) //right
            {
                leftright = 1;
            }
            else if ((arg & 0xFF00) == 0x0000) //left
            {
                leftright = 0;
            }
            gain  = arg & 0xFF;

            ExtCodec_SetGain(leftright, gain);
            mutex_unlock(&gextcodec_mutex);
            break;
        }
#if 1//added by jrd.lipeng for 13TAH01 EN50332
        case SET_EXTCODEC_PARAMS:
            cust_extcodec_params_config(arg);
            break;
#endif
#endif
        ...
        case AUDDRV_LOG_PRINT:
        {
            ...
            printk("Aud_LJ22M_Clk_cntr = 0x%x\n", Aud_LJ22M_Clk_cntr);
            printk("Aud_LJ24M_Clk_cntr = 0x%x\n", Aud_LJ24M_Clk_cntr);
            printk("AuddrvSpkStatus = 0x%x\n", AuddrvSpkStatus);
            Ana_Log_Print();
            Afe_Log_Print();
            break;
        }
        case AUDDRV_LOWLATENCY_MODE:
        {
            PRINTK_AUDDRV("AudDrv AUDDRV_LOWLATENCY_MODE = %lu\n", arg);
            ResetWriteWaitEvent();
            isLowLatencyMode = arg != 0;
            isModeChange = true;            
            break;
        }
        case AUDDRV_KERNEL_DEBUG_MODE:
        {
            PRINTK_AUDDRV("AudDrv AUDDRV_KERNEL_DEBUG_MODE = %lu\n", arg);
            isDebugMode = arg != 0;
            break;
        }
        case AUDDRV_ECHOREF_MODE:
        {
            PRINTK_AUDDRV("AudDrv AUDDRV_ECHOREF_MODE = %lu\n", arg);
            bEchoRefMode = arg != 0;
            isEchoRefModeChange = true;
            break;
        }
        ...
        case AUDDRV_GET_IPO_EVER:
        {
            printk("+AUDDRV_GET_IPO_EVER AudDrvSuspend_ipoh_Status = %d\n ", AudDrvSuspend_ipoh_Status);
            ret = AudDrvSuspend_ipoh_Status;
            break;
        }
        case AUDDRV_CLEAR_IPO_EVER:
        {
            printk("+AUDDRV_CLEAR_IPO_EVER\n");
            AudDrvSuspend_ipoh_Status = false;
            break;
        }
...
static ssize_t AudDrv_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
    ...
    int wait_count, live_threadshold;
    unsigned long long t1, t2, tin_loop,tout_loop;
    int Afe_consumed_bytes = 0;
    int timeout_counter = 0;
    int debug_loopcount = 0;

    tin_loop = sched_clock();
    ...
    PRINTK_DEBUG_MODE("AudDrv_write start write %d\n", count);  // (free  space of buffer)        
    ...
    while (count)
    {
        ...
        PRINTK_DEBUG_MODE("AudDrv_write start_copy %d\n", copy_size);
        ...
        if (isDebugMode)
        {
            Afe_consumed_bytes = (Afe_Get_Reg(AFE_DL1_CUR) - Afe_Block->u4DMAReadIdx - Afe_Block->pucPhysBufAddr);
            if (Afe_consumed_bytes < 0) { Afe_consumed_bytes += Afe_Block->u4BufferSize; }
            PRINTK_AUDDRV("+Auddrv debug_tool @@%d,%d,%d,%d,%d,%d,%d,%d\n",
                          1,
                          Afe_Block->u4DMAReadIdx,
                          Afe_Block->u4WriteIdx,
                          -1,
                          ...
                          Afe_consumed_bytes,
                          Afe_Get_Reg(AFE_DL1_CUR) - Afe_Block->pucPhysBufAddr,
                          copy_size);
        }
        ...
        do  //wait interrupt
        {
            ...
            if (isLowLatencyMode && pAfe_MEM_ConTrol == &AFE_dL1_Control_context)
            {
                int live_remained = Afe_Block->u4WriteIdx - (Afe_Get_Reg(AFE_DL1_CUR) - Afe_Block->pucPhysBufAddr);
                if (live_remained < 0) { live_remained += Afe_Block->u4BufferSize; }

                PRINTK_DEBUG_MODE("live_remained = %d ,Afe_Block->u4DMAReadIdx=%d, written_size =%d, DataRemained=%d\n",
                                  live_remained, Afe_Block->u4DMAReadIdx, written_size, Afe_Block->u4DataRemained);

                live_threadshold = written_size * 2;

                if (Afe_Block->u4DataRemained >= written_size &&
                    live_remained <= Afe_Block->u4DataRemained &&
                    live_remained >= live_threadshold)
                {
                    PRINTK_DEBUG_MODE("Afe_Block->DataRemained = %d > %d ,live remained = %d>%d, force wait this time\n",
                                      Afe_Block->u4DataRemained,
                                      written_size,
                                      live_remained,
                                      live_threadshold);
            ...
            t2 = t2 - t1; // in ns (10^9)
            PRINTK_DEBUG_MODE("AudDrv_write end_copy count=%d t=%llu\n", count, t2);
            SaveWriteWaitEvent((unsigned int)t2);
            CheckWriteWaitEvent();
            if ((ret <= 0) && (t2  > DL1_Interrupt_Interval * 1000000))
            {
                PRINTK_AUDDRV("AudDrv_write timeout, [Warning]blocked by others.(%llu)ns,(%d)jiffies written_size = %d, timoutcounter=%d\n",
                              t2, (DL1_Interrupt_Interval * 2 / 10), written_size, timeout_counter);
                if (timeout_counter++ > 1)  //allow 1 timeout for mode switch
                {
                    PRINTK_AUDDRV("auddrv write t2 = %llu t1 = %llu t2-t1 = %llu WriteArrayIndex = %d\n", t2, t1, t2 - t1, WriteArrayIndex);
    ...
    tout_loop = sched_clock();
    t2 = tout_loop - tin_loop;
    
    if (AudDebug_DLIrqUnderflow || (t2  > ((DL1_Interrupt_Interval<<2) * 1000000)))
    {
        PRINTK_AUDDRV("in = %llu out = %llu DIF = %llu CK %d underflow [%d]\n",tin_loop,tout_loop,t2,DL1_Interrupt_Interval,AudDebug_DLIrqUnderflow);
        PRINTK_AUDDRV("pin = %llu pout = %llu \n",prev_write_tin, prev_write_tout);
        PRINTK_AUDDRV("AWD %d %d\n",written_size,debug_loopcount);    
        //aee_kernel_exception_api(__FILE__, __LINE__, DB_OPT_FTRACE, "AudDrv_write Slow , data underflow", "AudDrv_write Slow , data underflow");   
    }
    AudDebug_DLIrqUnderflow = false;
    prev_write_tin = tin_loop;
    prev_write_tout = tout_loop;   
    PRINTK_DEBUG_MODE("AudDrv_write exit_write:%d \n", written_size);
    //PRINTK_AUDDRV("AWD %d %d\n",written_size,debug_loopcount);
    return written_size;
}
...
ssize_t AudDrv_MEMIF_Read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
    ...
    while (count)
    {
        ...
        if (count != 0)
        {
            if (isEchoRefModeChange)
            {
                AudDrv_getULInterval();
            }
            ...
            switch (pAfe_MEM_ConTrol->MemIfNum)
            {
                case MEM_VUL:
                {
                    ...
                    ret = wait_event_interruptible_timeout(VUL_Wait_Queue, VUL_wait_queue_flag, (UL_Interrupt_Interval * 2 / 10));
                    break;
                }
                case MEM_DAI:
                {
                    ...
                    ret = wait_event_interruptible_timeout(DAI_Wait_Queue, DAI_wait_queue_flag, (UL_Interrupt_Interval * 2 / 10));
                    break;
                }
                case MEM_AWB:
                {
                    ...
                    ret = wait_event_interruptible_timeout(AWB_Wait_Queue, AWB_wait_queue_flag, (UL_Interrupt_Interval * 2 / 10));
                    break;
                }
                case MEM_MOD_DAI:
                {
                    ...
                    ret = wait_event_interruptible_timeout(MODDAI_Wait_Queue, MODDAI_wait_queue_flag, (UL_Interrupt_Interval * 2 / 10));
                    break;
                }
                default:
                    printk("cannot find matcg MemIfNum!!!\n");
