...
#define AUXADC_HEADSET_ID_CHANNEL 12
/*----------------------------------------------------------------------
...
#if defined(DIABLOXPLUS_KK_IPHONE)
static int get_headset_adc(void)
{
	...
	for(i = 0; i < 5; i++)
	{
		...
		{	  
			printk("accdet [adc_driver]: channel1[%d]raw =%d\n",i,rawdata);
		}		 
		headset_id_vol += data[0]*1000+data[1]*10;
		printk("accdet  gyc  adc: channel1[%d]raw =%d\n",i,rawdata * 1800 / 4096);
	}
...
static int get_accdet_status()
{
	...
	if(iphone_headset == 1){
	 ...
	 if(700 < headset_id_vol && headset_id_vol < 1200){	 	
		iphone_headset = 1;
	 }
	ACCDET_DEBUG("[Accdet] gyc GPIO74 = %d,iphone_headset = %d\n",mt_get_gpio_out(GPIO74),iphone_headset);
	current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
	if(iphone_headset == 1)
		mt_set_gpio_out(GPIO74, GPIO_OUT_ONE);
	ACCDET_DEBUG("[Accdet] gyc  GPIO74 = %d,iphone_headset = %d\n",mt_get_gpio_out(GPIO74),iphone_headset);
...
#endif
#if defined(DIABLOXPLUS_KK_IPHONE)
static void detect_headset_status(void)
{
	int current_status = 0;
	int headset_id_vol = 0;
	
	headset_id_vol = get_headset_adc();
	ACCDET_DEBUG("[Accdet]  gyc  detect_headset_status  headset_id_vol = %d\n",headset_id_vol);
	 if(0 < headset_id_vol && headset_id_vol < 150){
	 	
		cable_type = HEADSET_NO_MIC;//pu tong san duan shi erji
		ACCDET_DEBUG("[Accdet]  gyc  cable_type = HEADSET_NO_MIC\n");
		
	 }else if(150 <  headset_id_vol && headset_id_vol < 800){
		
		cable_type = HEADSET_MIC;//pu tong si duan shi erji
		...
		msleep(500);
		headset_id_vol = get_headset_adc();
		ACCDET_DEBUG("[Accdet]  gyc  ihpone detect_headset_status  headset_id_vol = %d\n",headset_id_vol);
        ...
        case PLUG_OUT:
            ...
            if(current_status == 0)
            {
				...
				pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
				 ...
				 msleep(1000);
#if defined(DIABLOXPLUS_KK_IPHONE)
				 current_status = get_accdet_status(); //A=bit1; B=bit0
#else
				current_status = ((pmic_pwrap_read(ACCDET_STATE_RG) & 0xc0)>>6); //A=bit1; B=bit0
#endif
...
{ 
   ...
   // pmic_pwrap_write(ACCDET_STATE_SWCTRL, ACCDET_SWCTRL_EN);
	...
	mt_set_gpio_pull_enable(GPIO74, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO74, GPIO_PULL_UP);
...
static ssize_t show_headset_vol(struct device *dev,struct device_attribute *attr,char *buf)
{
	...
	int headset_id_vol = 0;
	  ...
	  headset_id_vol = data[0]*1000+data[1]*10;
	   printk("accdet  adc: channel1  headset_id_vol =%d     GPIO74 = %d\n",headset_id_vol,mt_get_gpio_out(GPIO74));
